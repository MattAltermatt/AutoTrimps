...
- **Phase 1 true-TS (milestone #5) — Waves 1–3 shipped** (2026-07-09) — `@ts-nocheck` → strict TS
  conversion via the esbuild byte-diff gate (emitted JS byte-identical to `gh-pages`; every fix is
  type-only). Wave 1 (#26 stance / #27 calc / #28 jobs+heirlooms+fight-info), Wave 2 (#29
  buildings/upgrades/magmite/gather/equipment), Wave 3 (#30 the nine orchestrators:
  scryer/nature/breedtimer/ab/portal/MAZ/maps/other/mapfunctions). The ambient seam (`trimps.d.ts`
  game API + `at-legacy.d.ts` AT globals) grows as readers convert. **Next: #31 fill** (`settings-*`
  / `import-export` / `utils` — the last `@ts-nocheck` modules) closes Phase 1. **#32** tracks latent
  faithful-port bugs surfaced by strict mode + preserved byte-faithfully via `@ts-expect-error` (e.g.
  portal `loom` undefined ref, mapfunctions `recyle` typo); fixes are tuning-gated + post-Phase-1.
