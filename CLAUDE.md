...
- `legacy/` — untouched original `.js`, the behavioral oracle. Only `AutoTrimps2.js` (loader)
  remains as first-party code (#133); the rest is third-party (`highcharts.js` — now dead, #134;
  `FastPriorityQueue.js`) or distribution shims. Graphs.js is ported (`src/modules/graphs/`, ECharts).
- `src/modules/` — the ~30 converted TypeScript modules (+ the `graphs/` directory module).
