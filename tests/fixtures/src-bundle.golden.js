...
(() => {
  ...
  __export(breedtimer_exports, {
    ...
    trimpsEffectivelyEmployed: () => trimpsEffectivelyEmployed2
  });
  MODULES["breedtimer"] = {};
  MODULES["breedtimer"].voidCheckPercent = 95;
  function trimpsEffectivelyEmployed2() {
  ...
  function breedingPS2() {
    var trimps = game.resources.trimps;
    var breeding = new DecimalBreed(trimps.owned).minus(trimpsEffectivelyEmployed2());
    return potencyMod().minus(1).mul(10).mul(breeding);
  }
  function potencyMod() {
    var trimps = game.resources.trimps;
    var potencyMod2 = new DecimalBreed(trimps.potency);
    if (game.upgrades.Potency.done > 0) potencyMod2 = potencyMod2.mul(Math.pow(1.1, game.upgrades.Potency.done));
    ...
    if (game.global.challengeActive == "Daily") {
      if (typeof game.global.dailyChallenge.dysfunctional !== "undefined")
        potencyMod2 = potencyMod2.mul(dailyModifiers.dysfunctional.getMult(game.global.dailyChallenge.dysfunctional.strength));
      if (typeof game.global.dailyChallenge.toxic !== "undefined")
        potencyMod2 = potencyMod2.mul(dailyModifiers.toxic.getMult(game.global.dailyChallenge.toxic.strength, game.global.dailyChallenge.toxic.stacks));
    }
    if (challengeActive("Toxicity") && game.challenges.Toxicity.stacks > 0)
      potencyMod2 = potencyMod2.mul(Math.pow(game.challenges.Toxicity.stackMult, game.challenges.Toxicity.stacks));
    if (challengeActive("Archaeology"))
      potencyMod2 = potencyMod2.mul(game.challenges.Archaeology.getStatMult("breed"));
    if (challengeActive("Quagmire"))
      potencyMod2 = potencyMod2.mul(game.challenges.Quagmire.getExhaustMult());
    if (game.global.voidBuff == "slowBreed")
      potencyMod2 = potencyMod2.mul(0.2);
    potencyMod2 = calcHeirloomBonusDecimal("Shield", "breedSpeed", potencyMod2);
  ...
  function ATGA2() {
    if (game.jobs.Geneticist.locked == false && getPageSetting2("ATGA2") == true && getPageSetting2("ATGA2timer") > 0 && game.global.challengeActive != "Trapper") {
      ...
      if (challengeActive("Archaeology")) potencyMod2 = potencyMod2.mul(game.challenges.Archaeology.getStatMult("breed"));
      if (challengeActive("Quagmire")) potencyMod2 = potencyMod2.mul(game.challenges.Quagmire.getExhaustMult());
      if (game.global.voidBuff == "slowBreed") {
        potencyMod2 = potencyMod2.mul(0.2);
      }
      potencyMod2 = calcHeirloomBonusDecimal("Shield", "breedSpeed", potencyMod2);
      if (game.jobs.Geneticist.owned > 0) potencyMod2 = potencyMod2.mul(Math.pow(0.98, game.jobs.Geneticist.owned));
      potencyMod2 = potencyMod2.div(10).add(1);
      var decimalOwned = missingTrimps.add(trimps.owned);
      var timeRemaining = DecimalBreed.log10(maxBreedable.div(decimalOwned.minus(trimpsEffectivelyEmployed2()))).div(DecimalBreed.log10(potencyMod2)).div(10);
      var currentSend = game.resources.trimps.getCurrentSend();
      var totalTime = DecimalBreed.log10(maxBreedable.div(maxBreedable.minus(currentSend))).div(DecimalBreed.log10(potencyMod2)).div(10);
      var target;
      if (getPageSetting2("ATGA2timer") > 0)
        target = new Decimal(getPageSetting2("ATGA2timer"));
      if (getPageSetting2("zATGA2timer") > 0 && getPageSetting2("ztATGA2timer") > 0 && game.global.world < getPageSetting2("zATGA2timer"))
        target = new Decimal(getPageSetting2("ztATGA2timer"));
      if (getPageSetting2("ATGA2timerz") > 0 && getPageSetting2("ATGA2timerzt") > 0 && game.global.world >= getPageSetting2("ATGA2timerz"))
        target = new Decimal(getPageSetting2("ATGA2timerzt"));
      if (game.global.runningChallengeSquared && getPageSetting2("cATGA2timer") > 0 && challengeActive("Electricity") == false && challengeActive("Toxicity") == false && challengeActive("Nom") == false)
      ...
      if (getPageSetting2("dhATGA2timer") > 0 && game.global.challengeActive == "Daily" && (typeof game.global.dailyChallenge.bogged !== "undefined" || typeof game.global.dailyChallenge.plague !== "undefined" || typeof game.global.dailyChallenge.pressure !== "undefined"))
        target = new Decimal(getPageSetting2("dhATGA2timer"));
      if (game.global.challengeActive != "Daily" && getPageSetting2("sATGA2timer") > 0 && isActiveSpireAT() == true)
      ...
      if ((getPageSetting2("dATGA2Auto") == 2 || getPageSetting2("dATGA2Auto") == 1 && disActiveSpireAT() && game.global.challengeActive == "Daily") && game.global.challengeActive == "Daily" && (typeof game.global.dailyChallenge.bogged !== "undefined" || typeof game.global.dailyChallenge.plague !== "undefined")) {
        ...
        target = new Decimal(Math.ceil(isNaN(atl) ? target : atl / 1e3 * ((game.portal.Agility.level ? 1e3 * Math.pow(1 - game.portal.Agility.modifier, game.portal.Agility.level) : 1e3) + (game.talents.hyperspeed2.purchased && game.global.world <= Math.floor((game.global.highestLevelCleared + 1) * 0.5) || game.global.mapExtraBonus == "fa") * -100 + game.talents.hyperspeed.purchased * -100)));
      }
      ...
      if (compareTime.cmp(target) < 0) {
        if (game.resources.food.owned * (getPageSetting2("ATGA2gen") / 100) < getNextGeneticistCost()) {
          return;
        } else if (timeRemaining.cmp(1) < 0 || target.minus((now - game.global.lastSoldierSentAt) / 1e3).cmp(timeRemaining) > 0) {
          if (genDif.cmp(0) > 0) {
            if (genDif.cmp(10) > 0) genDif = new Decimal(10);
            addGeneticist(genDif.toNumber());
          }
        }
      } else if (compareTime.add(thresh.mul(-1)).cmp(target) > 0 || potencyMod2.cmp(1) == 0) {
  ...
  function autoNatureTokens() {
    ...
    for (var nature in game.empowerments) {
      ...
      if (setting == "Empowerment") {
        ...
        empowerment.level++;
        changed = true;
        debug2("Upgraded Empowerment of " + nature, "nature");
      } else if (setting == "Transfer") {
        if (empowerment.retainLevel >= 80 || empowerment.tokens < thresh)
          continue;
        ...
        changed = true;
        debug2("Upgraded " + nature + " transfer rate", "nature");
  ...
  function autoEnlight() {
    ...
    if (game.global.challengeActive != "Daily" && !game.global.runningChallengeSquared) {
      if (getPageSetting2("pfillerenlightthresh") >= 0) {
        poison = game.empowerments.Poison.nextUberCost <= getPageSetting2("pfillerenlightthresh") && game.empowerments.Poison.nextUberCost <= game.empowerments.Poison.tokens;
        if (poison) {
          poisondiff = getPageSetting2("pfillerenlightthresh") - game.empowerments.Poison.nextUberCost;
        } else poisondiff = -999999;
      } else poisondiff = -999999;
      if (getPageSetting2("wfillerenlightthresh") >= 0) {
        wind = game.empowerments.Wind.nextUberCost <= getPageSetting2("wfillerenlightthresh") && game.empowerments.Wind.nextUberCost <= game.empowerments.Wind.tokens;
        if (wind) {
          winddiff = getPageSetting2("wfillerenlightthresh") - game.empowerments.Wind.nextUberCost;
        } else winddiff = -999999;
      } else winddiff = -999999;
      if (getPageSetting2("ifillerenlightthresh") >= 0) {
        ice = game.empowerments.Ice.nextUberCost <= getPageSetting2("ifillerenlightthresh") && game.empowerments.Ice.nextUberCost <= game.empowerments.Ice.tokens;
        if (ice) {
          icediff = getPageSetting2("ifillerenlightthresh") - game.empowerments.Ice.nextUberCost;
        } else icediff = -999999;
      } else icediff = -999999;
    ...
    if (game.global.challengeActive == "Daily") {
      if (getPageSetting2("pdailyenlightthresh") >= 0) {
        dpoison = game.empowerments.Poison.nextUberCost <= getPageSetting2("pdailyenlightthresh") && game.empowerments.Poison.nextUberCost <= game.empowerments.Poison.tokens;
        if (dpoison) {
          dpoisondiff = getPageSetting2("pdailyenlightthresh") - game.empowerments.Poison.nextUberCost;
        } else dpoisondiff = -999999;
      } else dpoisondiff = -999999;
      if (getPageSetting2("wdailyenlightthresh") >= 0) {
        dwind = game.empowerments.Wind.nextUberCost <= getPageSetting2("wdailyenlightthresh") && game.empowerments.Wind.nextUberCost <= game.empowerments.Wind.tokens;
        if (dwind) {
          dwinddiff = getPageSetting2("wdailyenlightthresh") - game.empowerments.Wind.nextUberCost;
        } else dwinddiff = -999999;
      } else dwinddiff = -999999;
      if (getPageSetting2("idailyenlightthresh") >= 0) {
        dice = game.empowerments.Ice.nextUberCost <= getPageSetting2("idailyenlightthresh") && game.empowerments.Ice.nextUberCost <= game.empowerments.Ice.tokens;
        if (dice) {
          dicediff = getPageSetting2("idailyenlightthresh") - game.empowerments.Ice.nextUberCost;
    ...
    if (game.global.runningChallengeSquared) {
      if (getPageSetting2("pc2enlightthresh") >= 0) {
        cpoison = game.empowerments.Poison.nextUberCost <= getPageSetting2("pc2enlightthresh") && game.empowerments.Poison.nextUberCost <= game.empowerments.Poison.tokens;
        if (cpoison) {
          cpoisondiff = getPageSetting2("pc2enlightthresh") - game.empowerments.Poison.nextUberCost;
        } else cpoisondiff = -999999;
      } else cpoisondiff = -999999;
      if (getPageSetting2("wc2enlightthresh") >= 0) {
        cwind = game.empowerments.Wind.nextUberCost <= getPageSetting2("wc2enlightthresh") && game.empowerments.Wind.nextUberCost <= game.empowerments.Wind.tokens;
        if (cwind) {
          cwinddiff = getPageSetting2("wc2enlightthresh") - game.empowerments.Wind.nextUberCost;
        } else cwinddiff = -999999;
      } else cwinddiff = -999999;
      if (getPageSetting2("ic2enlightthresh") >= 0) {
        cice = game.empowerments.Ice.nextUberCost <= getPageSetting2("ic2enlightthresh") && game.empowerments.Ice.nextUberCost <= game.empowerments.Ice.tokens;
        if (cice) {
          cicediff = getPageSetting2("ic2enlightthresh") - game.empowerments.Ice.nextUberCost;
  ...
  __export(calc_exports, {
    RcalcBadGuyDmg: () => RcalcBadGuyDmg2,
    RcalcDailyAttackMod: () => RcalcDailyAttackMod,
    ...
    calcEnemyAttackCore: () => calcEnemyAttackCore,
    calcEnemyBaseAttack: () => calcEnemyBaseAttack,
    calcEnemyBaseHealth: () => calcEnemyBaseHealth,
    calcEnemyHealth: () => calcEnemyHealth2,
    calcEnemyHealthCore: () => calcEnemyHealthCore,
    calcHDratio: () => calcHDratio2,
    calcOurBlock: () => calcOurBlock2,
    calcOurDmg: () => calcOurDmg2,
    calcOurHealth: () => calcOurHealth2,
    calcSpecificEnemyAttack: () => calcSpecificEnemyAttack2,
    ...
    highDamageShield: () => highDamageShield2,
    rCalcMutationAttack: () => rCalcMutationAttack,
    rCalcMutationHealth: () => rCalcMutationHealth,
    rMutationAttack: () => rMutationAttack,
    rMutationHealth: () => rMutationHealth,
    stormdynamicHD: () => stormdynamicHD2
  });
  globalThis.critCC = 1;
  globalThis.critDD = 1;
  ...
  function calcOurDmg2(minMaxAvg, incStance, incFlucts) {
    ...
    if (getPageSetting2("45stacks") == false && game.global.antiStacks > 0) {
      number *= game.global.antiStacks * game.portal.Anticipation.level * game.portal.Anticipation.modifier + 1;
    }
    if (getPageSetting2("45stacks") == true && game.global.antiStacks > 0) {
  ...
  function badGuyCritMult(enemy, critPower = 2, block, health) {
    if (getPageSetting2("IgnoreCrits") == 2) return 1;
  ...
  function calcEnemyHealth2(world, map) {
    ...
    if (game.global.spireActive) {
      health = calcSpire2(99, game.global.gridArray[99].name, "health");
    }
    return health;
  }
  ...
  function calcHDratio2(map) {
    ...
    if (!map || map < 1) {
      ratio = calcEnemyHealth2() / ourBaseDamage;
    }
    if (map || map >= 1)
      ratio = calcEnemyHealth2(map, true) / ourBaseDamage;
    return ratio;
  }
  ...
  function RcalcOurDmg2(minMaxAvg, equality, _unusedIncFlucts) {
    ...
    if (getPageSetting2("Rcalcfrenzy") == true) {
    ...
    } else if (getPageSetting2("Rcalcmaxequality") == 0 && !equality) {
  ...
  __export(equipment_exports, {
    ...
    equipfarmdynamicHD: () => equipfarmdynamicHD2,
    estimateEquipsForZone: () => estimateEquipsForZone2,
    evaluateEquipmentEfficiency: () => evaluateEquipmentEfficiency2,
    getMaxAffordable: () => getMaxAffordable2,
    getTotalMultiCost: () => getTotalMultiCost,
    mostEfficientEquipment: () => mostEfficientEquipment,
    orangewindstack: () => orangewindstack,
    postBuy3: () => postBuy32,
    preBuy3: () => preBuy32,
  ...
  function buyPrestigeMaybe(equipName) {
    ...
    ;
    if (game.upgrades[prestigeUpgradeName].cost.resources.science[0] * Math.pow(game.upgrades[prestigeUpgradeName].cost.resources.science[1], game.equipment[equipName].prestige - 1) > game.resources.science.owned) {
      return false;
    }
  ...
  function estimateEquipsForZone2() {
    ...
    if (healthNeededMulti < 1 && attackNeededMulti < 1) {
      return [0, {}];
    }
    ;
    let ourAttack = 6;
  ...
  function safeBuyBuilding2(building) {
    ...
    if (building === "Warpstation" && !game.buildings[building].locked && canAffordBuilding(building)) {
      ...
      debug2("Building " + game.global.buyAmt + " " + building + "s", "buildings", "*rocket");
      postBuy2(oldBuy);
      return;
    }
  ...
  function buyFoodEfficientHousing() {
    ...
    if (!ignoresLimit) {
      unlockedHousing = unlockedHousing.filter((b) => {
        if (game.buildings[b].owned < getPageSetting2("Max" + b) || getPageSetting2("Max" + b) < 1)
          return true;
  ...
  function buyBuildings() {
    ...
    if (!game.buildings.Gym.locked && (getPageSetting2("MaxGym") > game.buildings.Gym.owned || getPageSetting2("MaxGym") == -1)) {
      ...
      if (!(game.upgrades["Gymystic"].allowed - game.upgrades["Gymystic"].done > 0) && !skipGym) safeBuyBuilding2("Gym");
    }
  ...
  function safeBuyJob(jobTitle, amount) {
    ...
    if (result) {
      debug2((game.global.firing ? "Firing " : "Hiring ") + prettify(game.global.buyAmt) + " " + jobTitle + "s", "jobs", "*users");
  ...
  function buyJobs() {
    ...
    if (getPageSetting2("AutoMagmamancers") && stacks2 > tierMagmamancers) {
      ...
      postBuy2(old);
      debug2("Bought " + firesomedudes / inverse + " Magmamancers. Total Owned: " + game.jobs["Magmamancer"].owned, "magmite", "*users");
      tierMagmamancers += 1;
    ...
    if (game.resources.trimps.owned - game.resources.trimps.employed < 2) {
      ...
      if (c)
        safeFireJob("Miner", 2);
      if (a || b || c)
        debug2("Job Protection Triggered, Number Rounding Error: [f,l,m]= " + a + " " + b + " " + c, "other");
    }
  }
  ...
  function workerRatios() {
    ...
    } else if (game.buildings.Tribute.owned > 3e3 && mutations.Magma.active()) {
      ratioSet = MODULES["jobs"].autoRatio6;
    } else if (game.buildings.Tribute.owned > 1500) {
      ratioSet = MODULES["jobs"].autoRatio5;
    } else if (game.buildings.Tribute.owned > 1e3) {
      ratioSet = MODULES["jobs"].autoRatio4;
    } else if (game.resources.trimps.realMax() > 3e6) {
    ...
    setPageSetting2("FarmerRatio", ratioSet[0]);
    setPageSetting2("LumberjackRatio", ratioSet[1]);
    setPageSetting2("MinerRatio", ratioSet[2]);
  }
  ...
  function RworkerRatios() {
    ...
    setPageSetting2("RFarmerRatio", ratioSet[0]);
  ...
  __export(heirlooms_exports, {
    ...
    Rdhsmapstaffequip: () => Rdhsmapstaffequip,
    Rdhsshield1: () => Rdhsshield1,
    Rdhsshield2: () => Rdhsshield2,
    Rdhstributestaff: () => Rdhstributestaff,
    ...
    Rhsmapstaffequip: () => Rhsmapstaffequip,
    Rhsshield1: () => Rhsshield1,
    Rhsshield2: () => Rhsshield2,
    Rhstributestaff: () => Rhstributestaff,
    Rhstributestaffequip: () => Rhstributestaffequip,
    ...
    dlowdmgshield: () => dlowdmgshield,
    evaluateHeirloomMods2: () => evaluateHeirloomMods2,
  ...
  function getHeirloomEff(name, type) {
    if (type == "staff") {
      if (getPageSetting2("slot1modst") == name) return 5;
      else if (getPageSetting2("slot2modst") == name) return 5;
      else if (getPageSetting2("slot3modst") == name) return 5;
      else if (getPageSetting2("slot4modst") == name) return 5;
      else if (getPageSetting2("slot5modst") == name) return 5;
      else if (getPageSetting2("slot6modst") == name) return 5;
      else if (getPageSetting2("slot7modst") == name) return 5;
      else return 0;
    } else if (type == "shield") {
      if (getPageSetting2("slot1modsh") == name) return 5;
      else if (getPageSetting2("slot2modsh") == name) return 5;
      else if (getPageSetting2("slot3modsh") == name) return 5;
      else if (getPageSetting2("slot4modsh") == name) return 5;
      else if (getPageSetting2("slot5modsh") == name) return 5;
      else if (getPageSetting2("slot6modsh") == name) return 5;
      else if (getPageSetting2("slot7modsh") == name) return 5;
      else return 0;
    } else if (type == "core") {
      if (getPageSetting2("slot1modcr") == name) return 5;
      else if (getPageSetting2("slot2modcr") == name) return 5;
      else if (getPageSetting2("slot3modcr") == name) return 5;
      else if (getPageSetting2("slot4modcr") == name) return 5;
      else return 0;
    }
  }
  ...
  function autoheirlooms32() {
    if (!heirloomsShown && game.global.heirloomsExtra.length > 0) {
      ...
      if (getPageSetting2("typetokeep") == 1) {
      ...
      } else if (getPageSetting2("typetokeep") == 2) {
        while (game.global.heirloomsCarried.length < getMaxCarriedHeirlooms() && game.global.heirloomsExtra.length > 0) {
          worthOfHeirlooms3();
      ...
      } else if (getPageSetting2("typetokeep") == 3) {
        while (game.global.heirloomsCarried.length < getMaxCarriedHeirlooms() && game.global.heirloomsExtra.length > 0) {
          worthOfHeirlooms3();
      ...
      } else if (getPageSetting2("typetokeep") == 4) {
        while (game.global.heirloomsCarried.length < getMaxCarriedHeirlooms() && game.global.heirloomsExtra.length > 0) {
          worthOfHeirlooms3();
          if (worth3["Shield"].length > 0) {
            var carryshield = worth3["Shield"].shift();
  ...
  function betterAutoFight() {
    ...
    var breeding = game.resources.trimps.owned - trimpsEffectivelyEmployed2();
  ...
  function readyToSwitch(stance = "S") {
    ...
    var willSuicide = getPageSetting2("ScryerDieZ");
  ...
  function useScryerStance() {
    ...
    function autoStanceFunctionScryer() {
      if (getPageSetting2("AutoStance") == 3 || getPageSetting2("use3daily") == true && game.global.challengeActive == "Daily") windStance();
      else if (AutoStance == 1) autoStance();
      else if (AutoStance == 2) autoStance2();
    }
    var aboveMaxZone = getPageSetting2("ScryerMaxZone") > 0 && game.global.world >= getPageSetting2("ScryerMaxZone");
    var USS = getPageSetting2("UseScryerStance"), MA = game.global.mapsActive, SC = getPageSetting2("ScryerSkipCorrupteds2") == 0;
    ...
    never_scry |= USS && MA && getPageSetting2("ScryerUseinMaps2") == 0 && getCurrentMapObject().location != "Void" && getCurrentMapObject().location != "Bionic" && getCurrentMapObject().level <= game.global.world;
    never_scry |= USS && MA && getPageSetting2("ScryerUseinPMaps") == 0 && getCurrentMapObject().level > game.global.world && getCurrentMapObject().location != "Void" && getCurrentMapObject().location != "Bionic";
    never_scry |= USS && MA && getCurrentMapObject().location == "Void" && getPageSetting2("ScryerUseinVoidMaps2") == 0;
    never_scry |= USS && MA && getCurrentMapObject().location == "Bionic" && getPageSetting2("ScryerUseinBW") == 0;
    never_scry |= USS && !MA && (isActiveSpireAT() || disActiveSpireAT()) && getPageSetting2("ScryerUseinSpire2") == 0;
    never_scry |= USS && !MA && getPageSetting2("ScryerSkipBoss2") == 1 && game.global.world < getPageSetting2("VoidMaps") && game.global.lastClearedCell == 98;
    never_scry |= USS && !MA && getPageSetting2("ScryerSkipBoss2") == 0 && game.global.lastClearedCell == 98;
    never_scry |= USS && !MA && (getEmpowerment() == "Poison" && (getPageSetting2("ScryUseinPoison") == 0 || getPageSetting2("ScryUseinPoison") > 0 && game.global.world >= getPageSetting2("ScryUseinPoison")) || getEmpowerment() == "Wind" && (getPageSetting2("ScryUseinWind") == 0 || getPageSetting2("ScryUseinWind") > 0 && game.global.world >= getPageSetting2("ScryUseinWind")) || getEmpowerment() == "Ice" && (getPageSetting2("ScryUseinIce") == 0 || getPageSetting2("ScryUseinIce") > 0 && game.global.world >= getPageSetting2("ScryUseinIce")));
    ...
    var scryNext = !nextIsCorrupt && (transitionRequired || oneShotPower(void 0, 0, true));
    var skipOnMaxZone = getPageSetting2("onlyminmaxworld") == 2 && getPageSetting2("ScryerSkipCorrupteds2") != 1 && aboveMaxZone;
    ...
    use_scryer |= game.global.mapsActive && getCurrentMapObject().location == "Void" && (getPageSetting2("ScryerUseinVoidMaps2") == 1 || getPageSetting2("scryvoidmaps") == true && game.global.challengeActive != "Daily" || getPageSetting2("dscryvoidmaps") == true && game.global.challengeActive == "Daily");
    use_scryer |= game.global.mapsActive && getCurrentMapObject().location == "Bionic" && getPageSetting2("ScryerUseinBW") == 1;
    use_scryer |= game.global.mapsActive && getCurrentMapObject().level > game.global.world && getPageSetting2("ScryerUseinPMaps") == 1 && getCurrentMapObject().location != "Bionic";
    use_scryer |= !game.global.mapsActive && getPageSetting2("UseScryerStance") == true && (isActiveSpireAT() || disActiveSpireAT()) && getPageSetting2("ScryerUseinSpire2") == 1;
    ...
    var useOverkill = getPageSetting2("UseScryerStance") == true && getPageSetting2("ScryerUseWhenOverkill");
    useOverkill &= !(getPageSetting2("ScryerUseinSpire2") == 0 && !game.global.mapsActive && (isActiveSpireAT() || disActiveSpireAT()));
    ...
    var max_zone = getPageSetting2("ScryerMaxZone");
    var valid_min = game.global.world >= min_zone && game.global.world > 60;
    var valid_max = max_zone <= 0 || game.global.world < max_zone;
    if (USS && valid_min && valid_max && (!MA || getPageSetting2("onlyminmaxworld") == 0) && readyToSwitch()) {
  ...
  // src/modules/MAZ.ts
  var MAZ_exports = {};
  __export(MAZ_exports, {
    MAZLookalike: () => MAZLookalike,
    addRow: () => addRow,
    removeRow: () => removeRow,
  ...
  function MAZLookalike(titleText, _isItIn, _event) {
    ...
    tooltipText = "    <div id='windowContainer' style='display: block'><div id='windowError'></div>    <div class='row windowRow titles'>    <div class='windowCheckbox' style='width: 0%'></div>    <div class='windowZone'>Zone</div>";
    ...
    if (!game.global.preMapsActive && game.global.soldierHealth > 0) {
      ...
      for (critPower = 2; critPower >= -2; critPower--) {
        if (survive2("D", critPower)) {
          setFormation(2);
          break;
        } else if (survive2("XB", critPower)) {
          setFormation("0");
          break;
        } else if (survive2("B", critPower)) {
          setFormation(3);
          break;
        } else if (survive2("X", critPower)) {
          setFormation("0");
          break;
        } else if (survive2("H", critPower)) {
          setFormation(1);
          break;
        }
      }
  ...
  globalThis.enoughDamage = void 0;
  globalThis.enoughHealth = void 0;
  globalThis.shouldFarm = void 0;
  globalThis.RenoughDamage = void 0;
  globalThis.RenoughHealth = void 0;
  globalThis.RshouldFarm = void 0;
  globalThis.RneedToVoid = void 0;
  globalThis.RdoVoids = void 0;
  globalThis.Rshoulddoquest = void 0;
  globalThis.Rquestshieldzone = void 0;
  globalThis.Rquestequalityscale = void 0;
  globalThis.RshouldDoMaps = void 0;
  globalThis.Rshouldfragfarm = void 0;
  globalThis.Rshouldtimefarm = void 0;
  globalThis.Rdshouldtimefarm = void 0;
  globalThis.Rshouldsmithyfarm = void 0;
  globalThis.Rshouldtributefarm = void 0;
  globalThis.Rshoulddobogs = void 0;
  globalThis.Rshoulddopraid = void 0;
  globalThis.Rdshoulddopraid = void 0;
  globalThis.Rshouldinsanityfarm = void 0;
  globalThis.Rshouldalchfarm = void 0;
  globalThis.Rshouldhypofarm = void 0;
  globalThis.Rhyposhouldwood = void 0;
  globalThis.Rshouldstormfarm = void 0;
  globalThis.Rshoulddesofarm = void 0;
  globalThis.Rshouldequipfarm = void 0;
  globalThis.Rshouldshipfarm = void 0;
  globalThis.Rshouldmayhem = void 0;
  globalThis.Rshouldpanda = void 0;
  globalThis.RvanillaMAZ = void 0;
  globalThis.RdoMaxMapBonus = void 0;
  globalThis.Rinsanityfarm = void 0;
  globalThis.Rstormfarm = void 0;
  globalThis.Rdesofarm = void 0;
  globalThis.Rshipfarm = void 0;
  globalThis.Ralchfarm = void 0;
  globalThis.Rshouldcastle = void 0;
  globalThis.Rhypofarm = void 0;
  globalThis.Requipfarm = void 0;
  globalThis.RlastMapWeWereIn = void 0;
  MODULES.maps = {};
  ...
  MODULES.maps.preferGardens = !getPageSetting2("PreferMetal");
  ...
  function autoMap() {
    ...
    if (game.global.challengeActive == "Daily") {
      voidMapLevelSettingCell = getPageSetting2("dvoidscell") > 0 ? getPageSetting2("dvoidscell") : 70;
    }
    if (game.global.challengeActive != "Daily" && getPageSetting2("VoidMaps") > 0) {
      voidMapLevelSetting = getPageSetting2("VoidMaps");
    }
    if (game.global.challengeActive == "Daily" && getPageSetting2("DailyVoidMod") >= 1) {
      voidMapLevelSetting = getPageSetting2("DailyVoidMod");
    }
    if (getPageSetting2("RunNewVoidsUntilNew") != 0 && game.global.challengeActive != "Daily") {
      voidMapLevelPlus = getPageSetting2("RunNewVoidsUntilNew");
    }
    if (getPageSetting2("dRunNewVoidsUntilNew") != 0 && game.global.challengeActive == "Daily") {
    ...
    if (game.global.totalVoidMaps <= 0 || !needToVoid || getPageSetting2("novmsc2") == true && game.global.runningChallengeSquared || game.global.challengeActive != "Daily" && game.global.totalVoidMaps > 0 && getPageSetting2("onlystackedvoids") == true && voidArrayDoneS.length < 1) {
      doVoids = false;
    }
    ...
    if (game.global.mapBonus >= getPageSetting2("MaxMapBonuslimit") && !shouldFarm)
      ...
      shouldFarmLowerZone = getPageSetting2("LowerFarmingZone");
    else if (game.global.mapBonus < getPageSetting2("MaxMapBonushealth") && !enoughHealth && !shouldDoMaps && !needPrestige) {
      shouldDoMaps = true;
    ...
    if (!preSpireFarming && getPageSetting2("SkipSpires") == 1 && (game.global.challengeActive != "Daily" && isActiveSpireAT() || game.global.challengeActive == "Daily" && disActiveSpireAT())) {
      enoughDamage = true;
      enoughHealth = true;
      shouldFarm = false;
      shouldDoMaps = false;
      ...
      if (selectedMap2 == "world") {
        if (preSpireFarming) {
          ...
          if (game.global.world + extraMapLevels <= game.global.mapsOwnedArray[highestMap].level)
            selectedMap2 = game.global.mapsOwnedArray[highestMap].id;
          else
            selectedMap2 = "create";
        } else if (siphonMap != -1)
          selectedMap2 = game.global.mapsOwnedArray[siphonMap].id;
        else
          selectedMap2 = "create";
      }
    }
    ...
    if (!game.global.preMapsActive && game.global.mapsActive) {
      ...
      if (selectedMap2 == game.global.currentMapId && (!getCurrentMapObject().noRecycle && (doDefaultMapBonus || vanillaMapatZone || doMaxMapBonus || shouldFarm || needPrestige || shouldDoSpireMaps))) {
        ...
        if (shouldDoHealthMaps && game.global.mapBonus >= getPageSetting2("MaxMapBonushealth") - 1) {
          repeatClicked();
          shouldDoHealthMaps = false;
        }
        if (doMaxMapBonus && game.global.mapBonus >= getPageSetting2("MaxMapBonuslimit") - 1) {
          repeatClicked();
          doMaxMapBonus = false;
        }
      } else {
      ...
      if (selectedMap2 != "world") {
        ...
        if ((!getPageSetting2("PowerSaving") || getPageSetting2("PowerSaving") == 2 && doVoids) && game.global.switchToMaps && (needPrestige || doVoids || challengeActive("Lead") && !challSQ && game.global.world % 2 == 1 || !enoughDamage && enoughHealth && game.global.lastClearedCell < 9 || shouldFarm && game.global.lastClearedCell >= customVars.shouldFarmCell || scryerStuck) && (game.resources.trimps.realMax() <= game.resources.trimps.owned + 1 || challengeActive("Lead") && !challSQ && game.global.lastClearedCell > 93 || doVoids && game.global.lastClearedCell > 70)) {
          if (scryerStuck) {
            debug2("Got perma-stuck on cell " + (game.global.lastClearedCell + 2) + " during scryer stance. Are your scryer settings correct? Entering map to farm to fix it.");
          }
          mapsClicked();
        }
      }
      ...
      if (selectedMap2 == "world") {
        ...
        if (updateMapCost(true) > game.resources.fragments.owned) {
          if (game.jobs.Explorer.owned > 0 || game.unlocks.imps.Flutimp == true) {
            ...
            debug2("...selected our highest map instead # " + game.global.mapsOwnedArray[highestMap].id + " Level: " + game.global.mapsOwnedArray[highestMap].level, "maps", "*happy2");
            runMap();
          ...
          if (result == -2) {
            ...
            recycleBelow(true);
            debug2("Retrying, Buying a Map, level: #" + maplvlpicked, "maps", "th-large");
            result = buyMap();
            if (result == -2) {
              recycleMap(lowestMap);
              result = buyMap();
              if (result == -2)
                debug2("AutoMaps unable to recycle to buy map!");
              else
                debug2("Retrying map buy after recycling lowest level map");
            }
          }
        }
      } else {
  ...
  function RautoMap() {
    ...
    if (getPageSetting2("RRunNewVoidsUntilNew") != 0 && game.global.challengeActive != "Daily") {
      voidMapLevelPlus = getPageSetting2("RRunNewVoidsUntilNew");
    }
    if (getPageSetting2("RdRunNewVoidsUntilNew") != 0 && game.global.challengeActive == "Daily") {
    ...
    if (game.options.menu.mapAtZone.enabled && game.global.canMapAtZone) {
      ...
      if (RvanillaMAZ) {
        ...
        return RupdateAutoMapsStatus2();
      }
    }
    if (getPageSetting2("Rtimefarm") || game.global.challengeActive == "Daily" && getPageSetting2("Rdtimefarm") == 2) {
      RtimeFarm(true, false, false, false, false);
    }
    if (game.global.challengeActive == "Daily" && getPageSetting2("Rdtimefarm") == 1) {
      RtimeFarm(true, false, false, false, true);
    }
    if (getPageSetting2("Rsmithyfarm")) {
      RsmithyFarm(false);
    }
    if (getPageSetting2("Rtributefarm")) {
    ...
    if (!Rdshoulddopraid && (RdAMPrepMap1 != void 0 || RdAMPrepMap2 != void 0 || RdAMPrepMap3 != void 0 || RdAMPrepMap4 != void 0 || RdAMPrepMap5 != void 0)) {
      RAMPreset(true);
    }
    if (game.global.challengeActive == "Mayhem") {
      let Rdomayhem = false;
    ...
    if (game.global.challengeActive == "Storm") {
      Rstormfarm = getPageSetting2("Rstormon") == true && game.global.world > 5 && (game.global.challengeActive == "Storm" && getPageSetting2("Rstormzone") > 0 && getPageSetting2("RstormHD") > 0 && getPageSetting2("Rstormmult") > 0);
      if (Rstormfarm) {
        Rstorm(true);
      }
    }
    if (game.global.challengeActive == "Desolation") {
      Rdesofarm = getPageSetting2("Rdesoon") == true && game.global.world > 5 && (game.global.challengeActive == "Desolation" && getPageSetting2("Rdesozone") > 0 && getPageSetting2("RdesoHD") > 0 && getPageSetting2("Rdesomult") > 0);
      if (Rdesofarm) {
        Rdeso(true);
      }
    }
    ...
    if (runUniques || Rshoulddobogs || Rshouldcastle) {
      for (const map in game.global.mapsOwnedArray) {
        ...
        if (Rshoulddobogs && theMap.name == "The Black Bog") {
          ...
          if (getPageSetting2("Rprispalace") == true && theMap.name == "Prismatic Palace" && game.mapUnlocks.Prismalicious.canRunOnce) {
          ...
          if (game.global.challengeActive == "Hypothermia" && getPageSetting2("Rhypocastle") > 0 && theMap.name == "Frozen Castle" && game.global.world >= getPageSetting2("Rhypocastle")) {
            if (getPageSetting2("Rhypovoids") == true && game.global.totalVoidMaps <= 0) {
              selectedMap2 = theMap.id;
              break;
            }
            if (getPageSetting2("Rhypovoids") == false) {
              selectedMap2 = theMap.id;
              break;
            }
          }
          if (game.global.challengeActive != "Hypothermia" && getPageSetting2("Rfrozencastle") != -1 && theMap.name == "Frozen Castle" && game.global.world >= getPageSetting2("Rfrozencastle")[0] && game.global.lastClearedCell + 1 >= getPageSetting2("Rfrozencastle")[1]) {
            selectedMap2 = theMap.id;
            break;
          }
        }
    ...
    if (Rshoulddopraid) {
      if (selectedMap2 == "world") {
        selectedMap2 = "createp";
      }
    }
    if (Rdshoulddopraid) {
      if (selectedMap2 == "world") {
        selectedMap2 = "dcreatep";
      }
    }
    if (!Rshoulddopraid && !Rdshoulddopraid) selectedMap2 = RselectMap(selectedMap2);
    if (!game.global.preMapsActive && game.global.mapsActive) {
      RmapRepeat(selectedMap2, shouldDoHealthMaps, restartVoidMap);
    }
    if (Rshoulddoquest == 6) selectedMap2 = "world";
    else if (!game.global.preMapsActive && !game.global.mapsActive) {
      if (selectedMap2 != "world" && !game.global.switchToMaps) {
        mapsClicked();
      }
    } else if (game.global.preMapsActive) {
      if (selectedMap2 == "world") {
        mapsClicked();
      } else if (selectedMap2 == "createp") {
        RAMP();
      } else if (selectedMap2 == "dcreatep") {
        dRAMP();
      } else if (selectedMap2 == "create") {
        if (game.global.selectedMapPreset > 1) selectAdvMapsPreset(1);
  ...
  __export(mapfunctions_exports, {
    ...
    Rbogs: () => Rbogs2,
    Rdeso: () => Rdeso2,
    RdesoExtra: () => RdesoExtra,
    RequipExtra: () => RequipExtra,
    RfragCalc: () => RfragCalc,
    RfragCheck: () => RfragCheck,
    RfragMap: () => RfragMap2,
    Rhypo: () => Rhypo2,
    RhypoMap: () => RhypoMap2,
    Rinsanity: () => Rinsanity2,
    RinsanityMap: () => RinsanityMap2,
    RlevelMap: () => RlevelMap2,
    RmapLevelCalc: () => RmapLevelCalc,
    RmapRepeat: () => RmapRepeat2,
    Rmayhem: () => Rmayhem2,
    RmayhemExtra: () => RmayhemExtra,
    RminFragMap: () => RminFragMap,
    RpandaExtra: () => RpandaExtra,
    RquestMap: () => RquestMap2,
    RresetVars: () => RresetVars2,
    RselectDeso: () => RselectDeso,
    RselectFrag: () => RselectFrag,
    RselectMap: () => RselectMap2,
    RselectMayhem: () => RselectMayhem,
    RselectOther: () => RselectOther,
    RselectPanda: () => RselectPanda,
    RselectQuest: () => RselectQuest,
    RselectShip: () => RselectShip,
    RselectSmithy: () => RselectSmithy,
    Rship: () => Rship2,
    RshipMap: () => RshipMap2,
    Rshould: () => Rshould2,
    RsmithyCalc: () => RsmithyCalc2,
    RsmithyFarm: () => RsmithyFarm2,
    RsmithyFarmMap: () => RsmithyFarmMap2,
    Rstorm: () => Rstorm2,
  ...
  globalThis.insanityfragmappy = void 0;
  globalThis.insanityprefragmappy = void 0;
  ...
  globalThis.shipfragmappy = void 0;
  globalThis.shipprefragmappy = void 0;
  ...
  globalThis.alchfragmappy = void 0;
  globalThis.alchprefragmappy = void 0;
  ...
  globalThis.hypofragmappy = void 0;
  globalThis.hypoprefragmappy = void 0;
  globalThis.hypofragmappybought = false;
  function RresetVars2() {
    RshouldFarm = false;
    RdoVoids = false;
    RneedToVoid = false;
    RneedPrestige = false;
    ...
    insanityfragmappy = void 0;
    insanityprefragmappy = void 0;
    ...
    shipfragmappy = void 0;
    shipprefragmappy = void 0;
    shipfragmappybought = false;
    Ralchfarm = false;
    Rshouldalchfarm = false;
    Rshouldhypofarm = false;
    Ralchfragfarming = false;
    alchfragmappy = void 0;
    alchprefragmappy = void 0;
    alchfragmappybought = false;
    Rhypofarm = false;
    Rhyposhouldwood = true;
    Rshouldhypofarm = false;
    Rhypofragfarming = false;
    hypofragmappy = void 0;
    ...
    if (Rshouldfragfarm) {
      for (var map in game.global.mapsOwnedArray) {
        if (!game.global.mapsOwnedArray[map].noRecycle && game.global.world - 1 == game.global.mapsOwnedArray[map].level) {
          selectedMap2 = game.global.mapsOwnedArray[map].id;
          break;
        } else {
          selectedMap2 = "create";
        }
  ...
  function RfragCheck(what) {
    ...
    var special = "fa";
    if (what == "insanity") {
      frag = getPageSetting2("Rinsanityfarmfrag");
      farmzone = getPageSetting2("Rinsanityfarmzone");
      farmlevel = getPageSetting2("Rinsanityfarmlevel");
      selection = "Plentiful";
    } else if (what == "ship") {
      frag = getPageSetting2("Rshipfarmfrag");
      farmzone = getPageSetting2("Rshipfarmzone");
      farmlevel = getPageSetting2("Rshipfarmlevel");
      special = game.global.highestRadonLevelCleared > 83 ? "lsc" : "ssc";
      selection = game.global.farmlandsUnlocked ? "Farmlands" : "Plentiful";
    } else if (what == "alch") {
      frag = getPageSetting2("Ralchfarmfrag");
      farmzone = getPageSetting2("Ralchfarmzone");
      farmlevel = getPageSetting2("Ralchfarmlevel");
    } else if (what == "hypo") {
      frag = getPageSetting2("Rhypofarmfrag");
      farmzone = getPageSetting2("Rhypofarmzone");
      farmlevel = getPageSetting2("Rhypofarmlevel");
      special = "lwc";
    }
    var farmlevelindex = farmzone.indexOf(game.global.world);
    var levelzones2 = farmlevel[farmlevelindex];
    if (what == "alch") {
      selection = getPageSetting2("Ralchfarmselection")[farmlevelindex];
    }
    if (frag) cost = RminFragMap(selection, levelzones2, special);
  ...
  function RtimeFarm2(should, level, map, special, daily) {
    ...
    var time = ((/* @__PURE__ */ new Date()).getTime() - game.global.zoneStarted) / 1e3 / 60;
    ...
    if (should && timefarmzone.includes(game.global.world)) {
      ...
      if (!daily && game.global.challengeActive == "Daily" && getPageSetting2("Rdtimefarm") != 2) {
        Rshouldtimefarm = false;
      }
    }
  }
  function RtimeFarmMap2(daily) {
  ...
  function RsmithyFarm2(amount) {
    var smithyfarmzone = getPageSetting2("Rsmithyfarmzone");
    var smithyfarmindex = smithyfarmzone.indexOf(game.global.world);
    var smithyfarmcell = getPageSetting2("Rsmithyfarmcell")[smithyfarmindex];
    var smithyfarmsmithy = getPageSetting2("Rsmithyfarmamount");
  ...
  function RsmithyCalc2(level, selection, special, gather) {
    var smithys = game.buildings.Smithy.owned;
    var goal = RsmithyFarm2(true) - smithys;
    var afford = true;
  ...
  function RtributeFarm2(should, level, map, special) {
    var tributefarmzone = getPageSetting2("Rtributefarmzone");
  ...
  function Rbogs2() {
    var bogzone = getPageSetting2("Rblackbogzone");
    var bogamount = getPageSetting2("Rblackbogamount");
  ...
  function Rmayhem2() {
    var hits = getPageSetting2("Rmayhemabcut") > 0 ? getPageSetting2("Rmayhemabcut") : 100;
    var hitssurv = getPageSetting2("Rmayhemhcut") > 0 ? getPageSetting2("Rmayhemhcut") : 1;
    var enemyDamage = RcalcBadGuyDmg(null, RgetEnemyMaxAttack(game.global.world, 50, "Snimp", 1));
    if (game.challenges.Mayhem.stacks > 0 && getPageSetting2("Rmayhemattack") == true && RcalcHDratio() > hits) {
      Rshouldmayhem = 1;
    }
    if (game.challenges.Mayhem.stacks > 0 && getPageSetting2("Rmayhemhealth") == true && RcalcOurHealth() < hitssurv * enemyDamage) {
      Rshouldmayhem = 2;
    }
  }
  function RmayhemExtra() {
    var mayhemextra = 0;
    if (Rshouldmayhem > 0 && getPageSetting2("Rmayhemmap") == 2) {
      mayhemextra = 0;
      var health = RcalcOurHealth() * 2;
      var attack = RcalcOurDmg("avg", false, true);
      var boss = game.challenges.Mayhem.getBossMult();
      var hitsmap = getPageSetting2("Rmayhemamcut") > 0 ? getPageSetting2("Rmayhemamcut") : 100;
      var hitssurv = getPageSetting2("Rmayhemhcut") > 0 ? getPageSetting2("Rmayhemhcut") : 1;
      var mlevels = 6;
      var go = false;
      if (RcalcEnemyHealth(game.global.world + mlevels) / boss <= attack * (hitsmap * (mlevels + 1)) && RcalcBadGuyDmg(null, RgetEnemyMaxAttack(game.global.world + mlevels, 20, "Snimp", 1)) / boss * 1.3 * hitssurv <= health) {
        mayhemextra = mlevels;
        go = true;
      }
      if (!go) {
  ...
  function RpandaExtra() {
    var pandaextra = 1;
    if (Rshouldpanda == true && getPageSetting2("Rpandamaps") == true) {
      pandaextra = 1;
      var health = RcalcOurHealth() * 2;
      var attack = RcalcOurDmg("avg", false, true);
      ...
      var mlevels = 6;
      var go = false;
      if (RcalcEnemyHealth(game.global.world + mlevels) / boss * mult <= attack * (hitsmap * (mlevels + 1)) && RcalcBadGuyDmg(null, RgetEnemyMaxAttack(game.global.world + mlevels, 20, "Snimp", 1)) / boss * mult * 1.3 * hitssurv <= health) {
        pandaextra = mlevels;
        go = true;
      }
      if (!go) {
  ...
  function Rinsanity2(should, level, reset) {
    ...
    var insanityfarmlevel = getPageSetting2("Rinsanityfarmlevel");
    ...
    insanityfarmzone = getPageSetting2("Rinsanityfarmzone");
    insanityfarmstacks = getPageSetting2("Rinsanityfarmstack");
    var insanitystacksfarmindex = insanityfarmzone.indexOf(game.global.world);
  ...
  function RinsanityMap2() {
    var insanityfragcheck = true;
    if (getPageSetting2("Rinsanityfarmfrag") == true) {
      if (RfragCheck("insanity") == true) {
        ...
        if (!insanityfragcheck && insanityfragmappy == void 0 && !insanityfragmappybought && game.global.preMapsActive && Rshouldinsanityfarm) {
        ...
        if (!insanityfragcheck && game.global.preMapsActive && !game.global.mapsActive && insanityfragmappybought && insanityfragmappy != void 0 && Rshouldinsanityfarm) {
          debug2("running insanity frag map");
        ...
        if (!insanityfragcheck && game.global.mapsActive && insanityfragmappybought && insanityprefragmappy != void 0 && Rshouldinsanityfarm) {
          if (RfragCheck("insanity") == false) {
            ...
            if (game.global.preMapsActive && insanityfragmappybought && insanityprefragmappy != void 0 && Rshouldinsanityfarm) {
              insanityfragmappybought = false;
            }
            if (insanityprefragmappy != void 0) {
    ...
    if (insanityfragcheck && getPageSetting2("Rinsanityfarmlevel") != 0) {
  ...
  function Rstorm2(should) {
    var stormzone = getPageSetting2("Rstormzone");
    var stormHD = getPageSetting2("RstormHD");
    var stormmult = getPageSetting2("Rstormmult");
  ...
  function Rdeso2(should) {
    var desozone = getPageSetting2("Rdesozone");
    var desoHD = getPageSetting2("RdesoHD");
    var desomult = getPageSetting2("Rdesomult");
  ...
  function RdesoExtra() {
    ...
    if (Rshoulddesofarm == true) {
      ...
      var mlevels = 6;
      var go = false;
      if (RcalcEnemyHealth(game.global.world + mlevels) <= attack * (hitsmap * (mlevels + 1)) && RcalcBadGuyDmg(null, RgetEnemyMaxAttack(game.global.world + mlevels, 20, "Snimp", 1)) * 1.3 * hitssurv <= health) {
        desoextra = mlevels;
        go = true;
      }
      if (!go) {
  ...
  function Rship2(should, level, reset) {
    var shipfarmzone = getPageSetting2("Rshipfarmzone");
    var shipamountfarmindex = shipfarmzone.indexOf(game.global.world);
    var shipfarmlevel = getPageSetting2("Rshipfarmlevel");
    if (level) return shipfarmlevel[shipamountfarmindex];
    var shipfarmamount = getPageSetting2("Rshipfarmamount");
    var ships = game.jobs.Worshipper.owned;
    var shipamountzones = shipfarmamount[shipamountfarmindex];
    if (getPageSetting2("Rshipfarmamount") == 50) shipamountzones = 50;
  ...
  function RshipMap2() {
    ...
    if (getPageSetting2("Rshipfarmfrag") == true) {
      if (RfragCheck("ship") == true) {
        ...
        shipfragcheck = false;
        if (!shipfragcheck && shipfragmappy == void 0 && !shipfragmappybought && game.global.preMapsActive && Rshouldshipfarm) {
        ...
        if (!shipfragcheck && game.global.preMapsActive && !game.global.mapsActive && shipfragmappybought && shipfragmappy != void 0 && Rshouldshipfarm) {
          debug2("running ship frag map");
        ...
        if (!shipfragcheck && game.global.mapsActive && shipfragmappybought && shipprefragmappy != void 0 && Rshouldshipfarm) {
          if (RfragCheck("ship") == false) {
            ...
            if (game.global.preMapsActive && shipfragmappybought && shipprefragmappy != void 0 && Rshouldshipfarm) {
              shipfragmappybought = false;
            }
            if (shipprefragmappy != void 0) {
    ...
    if (shipfragcheck && getPageSetting2("Rshipfarmlevel") != 0) {
      var shiplevelzones = Rship2(false, true, false);
      if (Rshouldshipfarm) {
  ...
  function RalchMap2() {
    ...
    if (getPageSetting2("Ralchfarmfrag") == true) {
      if (RfragCheck("alch") == true) {
        ...
        alchfragcheck = false;
        if (!alchfragcheck && alchfragmappy == void 0 && !alchfragmappybought && game.global.preMapsActive && Rshouldalchfarm) {
        ...
        if (!alchfragcheck && game.global.preMapsActive && !game.global.mapsActive && alchfragmappybought && alchfragmappy != void 0 && Rshouldalchfarm) {
          debug2("running alch frag map");
        ...
        if (!alchfragcheck && game.global.mapsActive && alchfragmappybought && alchprefragmappy != void 0 && Rshouldalchfarm) {
          if (RfragCheck("alch") == false) {
            ...
            if (game.global.preMapsActive && alchfragmappybought && alchprefragmappy != void 0 && Rshouldalchfarm) {
              alchfragmappybought = false;
            }
            if (alchprefragmappy != void 0) {
    ...
    if (alchfragcheck && getPageSetting2("Ralchfarmlevel") != 0) {
      if (Rshouldalchfarm) {
        var alchfarmzone = getPageSetting2("Ralchfarmzone");
        var alchfarmselection = getPageSetting2("Ralchfarmselection");
        var alchlevelzones = Ralch2(false, true, false);
  ...
  function Rhypo2(should, level, reset) {
    ...
    woodmax = calcHeirloomBonus("Shield", "storageSize", woodmax, false);
    var hypofarmamount = getPageSetting2("Rhypofarmstack");
  ...
  function RhypoMap2() {
    ...
    if (getPageSetting2("Rhypofarmfrag") == true) {
      if (RfragCheck("hypo") == true) {
        ...
        hypofragcheck = false;
        if (!hypofragcheck && hypofragmappy == void 0 && !hypofragmappybought && game.global.preMapsActive && Rshouldhypofarm) {
        ...
        if (!hypofragcheck && game.global.preMapsActive && !game.global.mapsActive && hypofragmappybought && hypofragmappy != void 0 && Rshouldhypofarm) {
          debug2("running hypo frag map");
        ...
        if (!hypofragcheck && game.global.mapsActive && hypofragmappybought && hypoprefragmappy != void 0 && Rshouldhypofarm) {
          if (RfragCheck("hypo") == false) {
            ...
            if (game.global.preMapsActive && hypofragmappybought && hypoprefragmappy != void 0 && Rshouldhypofarm) {
              hypofragmappybought = false;
            }
            if (hypoprefragmappy != void 0) {
    ...
    if (hypofragcheck && getPageSetting2("Rhypofarmlevel") != 0) {
  ...
  function RequipExtra() {
    ...
    if (Rshouldequipfarm) {
      ...
      var health = RcalcOurHealth() * 2;
      var attack = RcalcOurDmg("avg", false, true);
      var hits = getPageSetting2("Requipfarmhits") > 0 ? getPageSetting2("Requipfarmhits") : 10;
      var hitssurv = getPageSetting2("Rhitssurvived") > 0 ? getPageSetting2("Rhitssurvived") : 1;
      var mlevels = 0;
      var go = false;
      if (RcalcEnemyHealth(game.global.world + mlevels) <= attack * hits && RcalcBadGuyDmg(null, RgetEnemyMaxAttack(game.global.world + mlevels, 20, "Snimp", 1)) * 0.8 * hitssurv <= health) {
        equipminus = mlevels;
        go = true;
      }
      if (!go) {
  ...
  function Rshould2(any, one) {
    if (any) {
      if (!Rshoulddopraid && !Rdshoulddopraid && (RshouldDoMaps || RdoVoids || Rshouldfragfarm || Rshouldtimefarm || Rdshouldtimefarm || Rshouldsmithyfarm || Rshouldtributefarm || Rshoulddoquest > 0 || Rshouldmayhem > 0 || Rshouldpanda || Rshouldinsanityfarm || Rshouldstormfarm || Rshoulddesofarm || Rshouldequipfarm || Rshouldshipfarm || Rshouldalchfarm || Rshouldhypofarm)) return true;
      else return false;
    }
    var should = "no";
  ...
  function RselectMayhem() {
    var selectedMap2 = "create";
    if (getPageSetting2("Rmayhemmap") == 2) {
      for (var map in game.global.mapsOwnedArray) {
  ...
  function RselectPanda() {
    var selectedMap2 = "create";
    if (getPageSetting2("Rpandamaps") == true) {
      for (var map in game.global.mapsOwnedArray) {
  ...
  function RselectShip() {
    var selectedMap2 = "create";
    var level = getPageSetting2("Rshipfarmlevel");
    var levelzones2 = Rship2(false, true, false);
    var special = game.global.highestRadonLevelCleared > 83 ? "lsc" : "ssc";
    if (level == 0) {
      for (var map in game.global.mapsOwnedArray) {
  ...
  function RselectOther(other) {
    ...
    var levelzones2 = 0;
    if (other == "insanity") {
      level = getPageSetting2("Rinsanityfarmlevel");
      levelzones2 = Rinsanity2(false, true, false);
    } else if (other == "alch") {
      level = getPageSetting2("Ralchfarmlevel");
      levelzones2 = Ralch2(false, true, false);
    } else if (other == "hypo") {
      level = getPageSetting2("Rhypofarmlevel");
      levelzones2 = Rhypo2(false, true, false);
    } else if (other == "time") {
      level = getPageSetting2("Rtimefarmlevel");
      levelzones2 = RtimeFarm2(false, true, false, false, false);
    } else if (other == "dtime") {
      level = getPageSetting2("Rdtimefarmlevel");
      levelzones2 = RtimeFarm2(false, true, false, false, true);
    } else if (other == "tribute") {
      level = getPageSetting2("Rtributefarmlevel");
      levelzones2 = RtributeFarm2(false, true, false, false);
    }
    if (level == 0) {
      for (var map in game.global.mapsOwnedArray) {
        if (!game.global.mapsOwnedArray[map].noRecycle && game.global.world == game.global.mapsOwnedArray[map].level) {
  ...
  function RselectMap2(selectedMap2) {
    if (Rshould2(true, false) && selectedMap2 == "world") {
      if (Rshould2(false, true) == "frag") {
        selectedMap2 = RselectFrag();
      } else if (Rshould2(false, true) == "mayhem") {
        selectedMap2 = RselectMayhem();
      } else if (Rshould2(false, true) == "panda") {
        selectedMap2 = RselectPanda();
      } else if (Rshould2(false, true) == "deso") {
        selectedMap2 = RselectDeso();
      } else if (Rshould2(false, true) == "insanity") {
        selectedMap2 = RselectOther("insanity");
      } else if (Rshould2(false, true) == "alch") {
        selectedMap2 = RselectOther("alch");
      } else if (Rshould2(false, true) == "hypo") {
        selectedMap2 = RselectOther("hypo");
      } else if (Rshould2(false, true) == "ship") {
        selectedMap2 = RselectShip();
      } else if (Rshould2(false, true) == "time") {
        selectedMap2 = RselectOther("time");
      } else if (Rshould2(false, true) == "dtime") {
        selectedMap2 = RselectOther("dtime");
      } else if (Rshould2(false, true) == "smithy") {
        selectedMap2 = RselectSmithy();
      } else if (Rshould2(false, true) == "tribute") {
        selectedMap2 = RselectOther("tribute");
      } else if (Rshould2(false, true) == "quest") {
        selectedMap2 = RselectQuest();
      } else if (Rshould2(false, true) == "equip") {
        for (var map in game.global.mapsOwnedArray) {
  ...
  function RmapRepeat2(selectedMap2, shouldDoHealthMaps, restartVoidMap) {
    var doDefaultMapBonus = game.global.mapBonus < getPageSetting2("RMaxMapBonuslimit") - 1;
    if (RvanillaMAZ || (Rshoulddopraid || Rshoulddopraid && RAMPfragfarming) || (Rdshoulddopraid || Rdshoulddopraid && RdAMPfragfarming) || (Rshouldinsanityfarm || Rshouldinsanityfarm && Rinsanityfragfarming) || (Rshouldalchfarm || Rshouldalchfarm && Ralchfragfarming) || (Rshouldhypofarm || Rshouldhypofarm && Rhypofragfarming) || (Rshouldshipfarm || Rshouldshipfarm && Rshipfragfarming) || selectedMap2 == game.global.currentMapId && (!getCurrentMapObject().noRecycle && (doDefaultMapBonus || RvanillaMAZ || RdoMaxMapBonus || RshouldFarm || Rshouldfragfarm || Rshouldtimefarm || Rdshouldtimefarm || Rshouldsmithyfarm || Rshouldtributefarm || Rshoulddobogs || Rshoulddoquest > 0 || Rshouldmayhem > 0 || Rshouldpanda || Rshouldstormfarm || Rshoulddesofarm || Rshouldequipfarm))) {
      if (!game.global.repeatMap) {
        repeatClicked();
      }
      if (Rshoulddopraid && !RAMPfragfarming || Rdshoulddopraid && !RdAMPfragfarming) {
        if (game.options.menu.repeatUntil.enabled != 2) {
          game.options.menu.repeatUntil.enabled = 2;
        }
      } else if (Rshoulddopraid && RAMPfragfarming || Rdshoulddopraid && RdAMPfragfarming || Rshouldinsanityfarm && Rinsanityfragfarming || Rshouldalchfarm && Ralchfragfarming || Rshouldhypofarm && Rhypofragfarming || Rshouldshipfarm && Rshipfragfarming) {
        if (game.options.menu.repeatUntil.enabled != 0) {
          game.options.menu.repeatUntil.enabled = 0;
        }
      }
      if (!Rshoulddopraid && !RAMPfragfarming && !Rshouldfragfarm && !Rdshoulddopraid && !RdAMPfragfarming && !Rshouldinsanityfarm && !Rinsanityfragfarming && !Rshouldalchfarm && !Rshouldhypofarm && !Rhypofragfarming && !Ralchfragfarming && !Rshoulddobogs && !RshouldDoMaps && !Rshouldtimefarm && !Rdshouldtimefarm && !Rshouldsmithyfarm && !Rshouldtributefarm && Rshoulddoquest <= 0 && Rshouldmayhem <= 0 && !Rshouldpanda && !Rshouldstormfarm && !Rshoulddesofarm && !Rshouldequipfarm && !Rshouldshipfarm && !Rshipfragfarming) {
        repeatClicked();
      }
      if (shouldDoHealthMaps && game.global.mapBonus >= getPageSetting2("RMaxMapBonushealth")) {
        repeatClicked();
        shouldDoHealthMaps = false;
      }
      if (RdoMaxMapBonus && game.global.mapBonus < getPageSetting2("RMaxMapBonuslimit")) {
        repeatClicked();
        RdoMaxMapBonus = false;
      }
  ...
  function dailyAutoPortal() {
    ...
    if (getPageSetting2("AutoPortalDaily") == 1) {
      var OKtoPortal = false;
      if (!game.global.runningChallengeSquared) {
        var minZone = getPageSetting2("dHeHrDontPortalBefore");
        game.stats.bestHeliumHourThisRun.evaluate();
        var bestHeHr = game.stats.bestHeliumHourThisRun.storedValue;
        var bestHeHrZone = game.stats.bestHeliumHourThisRun.atZone;
        var myHeliumHr = game.stats.heliumHour.value();
  ...
  function doPortal(challenge) {
    ...
    if (getPageSetting2("AutoAllocatePerks") == 2) {
      ...
      activateClicked();
      cancelPortal();
      debug2("First Stage: Bought Max Looting II");
    }
    portalClicked();
    if (!portalWindowOpen) {
      portalClicked();
    }
    if (portalWindowOpen && getPageSetting2("AutoAllocatePerks") == 1 && typeof AutoPerks?.clickAllocate === "function") {
    ...
    if (portalWindowOpen && getPageSetting2("AutoStartDaily") == true && c2done) {
      if (getPageSetting2("u2daily") == true && portalUniverse == 1) {
        swapPortalUniverse();
      }
      ...
      if (lastUndone == 1) {
        debug2("All available Dailies already completed.", "portal");
        if (getPageSetting2("u1daily") == true && portalUniverse == 1 && challenge == autoTrimpSettings.RdHeliumHourChallenge.selected || getPageSetting2("u2daily") == true && portalUniverse == 2) {
          swapPortalUniverse();
        }
        selectChallenge(challenge || 0);
      } else {
        getDailyChallenge(lastUndone);
        debug2("Portaling into Daily for: " + getDailyTimeString(lastUndone, true) + " now!", "portal");
      }
    } else if (portalWindowOpen && challenge && c2done) {
      if (getPageSetting2("u1daily") == true && portalUniverse == 1 && challenge == autoTrimpSettings.RdHeliumHourChallenge.selected) {
        swapPortalUniverse();
      }
      selectChallenge(challenge);
    }
    if (portalWindowOpen && getPageSetting2("AutoAllocatePerks") == 2 && !game.portal.Looting_II.locked) {
      numTab(6, true);
      buyPortalUpgrade("Looting_II");
  ...
  function RautoPortal() {
    ...
    switch (autoTrimpSettings.RAutoPortal.selected) {
      case "Radon Per Hour":
        var OKtoPortal = false;
        if (!game.global.runningChallengeSquared) {
          var minZone = getPageSetting2("RnHrDontPortalBefore");
          game.stats.bestHeliumHourThisRun.evaluate();
          var bestHeHr = game.stats.bestHeliumHourThisRun.storedValue;
          var bestHeHrZone = game.stats.bestHeliumHourThisRun.atZone;
          var myHeliumHr = game.stats.heliumHour.value();
          var heliumHrBuffer = Math.abs(getPageSetting2("RadonHrBuffer"));
          if (!aWholeNewWorld)
  ...
  function RdailyAutoPortal() {
    ...
    if (getPageSetting2("RAutoPortalDaily") == 1) {
      var OKtoPortal = false;
      if (!game.global.runningChallengeSquared) {
        var minZone = getPageSetting2("RdHeHrDontPortalBefore");
        game.stats.bestHeliumHourThisRun.evaluate();
        var bestHeHr = game.stats.bestHeliumHourThisRun.storedValue;
        var bestHeHrZone = game.stats.bestHeliumHourThisRun.atZone;
        var myHeliumHr = game.stats.heliumHour.value();
  ...
  function RdoPortal(challenge) {
    ...
    if (getPageSetting2("RAutoAllocatePerks") == 2) {
      viewPortalUpgrades();
      numTab(6, true);
      if (getPageSetting2("Rdumpgreed") == true) {
        buyPortalUpgrade("Greed");
        debug2("First Stage: Bought Max Greed");
      } else {
        buyPortalUpgrade("Looting");
        debug2("First Stage: Bought Max Looting");
      }
      activateClicked();
      cancelPortal();
    }
    ...
    if (portalWindowOpen && getPageSetting2("RAutoAllocatePerks") == 1 && typeof RAutoPerks?.clickAllocate === "function") {
      RAutoPerks.clickAllocate();
    }
    if (portalWindowOpen && getPageSetting2("RAutoStartDaily") == true) {
      if (getPageSetting2("u1daily") == true && portalUniverse == 2) {
        swapPortalUniverse();
      }
      selectChallenge("Daily");
      checkCompleteDailies();
      ...
      if (lastUndone == 1) {
        debug2("All available Dailies already completed.", "portal");
        if (getPageSetting2("u2daily") == true && portalUniverse == 2 && challenge == autoTrimpSettings.dHeliumHourChallenge.selected || getPageSetting2("u1daily") == true && portalUniverse == 1) {
          swapPortalUniverse();
        }
        selectChallenge(challenge || 0);
      } else {
      ...
      if (getPageSetting2("u2daily") == true && portalUniverse == 2 && challenge == autoTrimpSettings.dHeliumHourChallenge.selected) {
        swapPortalUniverse();
      }
      selectChallenge(challenge);
    }
    if (portalWindowOpen && getPageSetting2("RAutoAllocatePerks") == 2) {
      numTab(6, true);
      if (getPageSetting2("Rdumpgreed") == true) {
        buyPortalUpgrade("Greed");
        debug2("Second Stage: Bought Max Greed");
      } else {
        buyPortalUpgrade("Looting");
        debug2("Second Stage: Bought Max Looting");
      }
    }
    RresetVars();
    activatePortal();
  ...
  __export(import_export_exports, {
    ...
    initializeSettingsProfiles: () => initializeSettingsProfiles,
    loadAutoTrimps: () => loadAutoTrimps,
  ...
  function settingsProfileMakeGUI2() {
  ...
  function ImportExportTooltip(what, event2) {
    ...
    if (what == "ExportAutoTrimps") {
      ...
      var c2listcolor = c2listcolor2, Rc2listcolor = Rc2listcolor2;
      var c2list = {
        Size: {
          number: 1,
  ...
  __export(query_exports, {
    ...
    getCurrentEnemy: () => getCurrentEnemy2,
    getEnemyMaxAttack: () => getEnemyMaxAttack2,
    getEnemyMaxHealth: () => getEnemyMaxHealth2,
    getPerSecBeforeManual: () => getPerSecBeforeManual2,
  ...
  globalThis.dprestraid = false;
  globalThis.failpraid = false;
  globalThis.dfailpraid = false;
  globalThis.bwraided = false;
  globalThis.dbwraided = false;
  globalThis.failbwraid = false;
  globalThis.dfailbwraid = false;
  globalThis.perked = false;
  globalThis.prestraidon = false;
  globalThis.dprestraidon = false;
  globalThis.mapbought = false;
  globalThis.dmapbought = false;
  globalThis.bwraidon = false;
  globalThis.dbwraidon = false;
  globalThis.presteps = null;
  globalThis.minMaxMapCost = void 0;
  globalThis.fMap = void 0;
  globalThis.pMap = void 0;
  globalThis.shouldFarmFrags = false;
  globalThis.praidDone = false;
  function armydeath() {
    if (game.global.mapsActive) return false;
    var e = game.global.lastClearedCell + 1, l = game.global.gridArray[e].attack * dailyModifiers.empower.getMult(game.global.dailyChallenge.empower.strength, game.global.dailyChallenge.empower.stacks), a = game.global.soldierHealth + game.global.soldierEnergyShield;
  ...
  function autoGoldenUpgradesAT(setting) {
    ...
    if (setting == "Helium")
      setting2 = "Helium";
    if (!game.global.dailyChallenge.seed && !game.global.runningChallengeSquared && autoTrimpSettings.AutoGoldenUpgrades.selected == "Helium" && getPageSetting2("radonbattle") > 0 && game.goldenUpgrades.Helium.purchasedAt.length >= getPageSetting2("radonbattle") || game.global.dailyChallenge.seed && autoTrimpSettings.dAutoGoldenUpgrades.selected == "Helium" && getPageSetting2("dradonbattle") > 0 && game.goldenUpgrades.Helium.purchasedAt.length >= getPageSetting2("dradonbattle"))
      setting2 = "Battle";
    if (setting == "Battle")
      setting2 = "Battle";
    if (!game.global.dailyChallenge.seed && !game.global.runningChallengeSquared && autoTrimpSettings.AutoGoldenUpgrades.selected == "Battle" && getPageSetting2("battleradon") > 0 && game.goldenUpgrades.Battle.purchasedAt.length >= getPageSetting2("battleradon") || game.global.dailyChallenge.seed && autoTrimpSettings.dAutoGoldenUpgrades.selected == "Battle" && getPageSetting2("dbattleradon") > 0 && game.goldenUpgrades.Battle.purchasedAt.length >= getPageSetting2("dbattleradon"))
      setting2 = "Helium";
    if (setting == "Void" || setting == "Void + Battle")
      setting2 = "Void";
    ...
    if (!success && setting2 == "Void") {
      ...
      if (autoTrimpSettings.AutoGoldenUpgrades.selected == "Void" && !game.global.dailyChallenge.seed && !game.global.runningChallengeSquared || autoTrimpSettings.dAutoGoldenUpgrades.selected == "Void" && game.global.dailyChallenge.seed)
        setting2 = "Helium";
      if (autoTrimpSettings.AutoGoldenUpgrades.selected == "Void" && getPageSetting2("voidheliumbattle") > 0 && game.global.world >= getPageSetting2("voidheliumbattle") || autoTrimpSettings.dAutoGoldenUpgrades.selected == "Void" && getPageSetting2("dvoidheliumbattle") > 0 && game.global.world >= getPageSetting2("dvoidheliumbattle") || (autoTrimpSettings.AutoGoldenUpgrades.selected == "Void + Battle" && !game.global.dailyChallenge.seed && !game.global.runningChallengeSquared || autoTrimpSettings.dAutoGoldenUpgrades.selected == "Void + Battle" && game.global.dailyChallenge.seed || autoTrimpSettings.cAutoGoldenUpgrades.selected == "Void + Battle" && game.global.runningChallengeSquared))
        setting2 = "Battle";
      buyGoldenUpgrade(setting2);
    }
  }
  ...
  function pcheck1() {
    ...
    var I;
    if (game.global.challengeActive != "Daily") {
      HD = getPageSetting2("PraidingHD");
      P = getPageSetting2("PraidingP") > 0 ? getPageSetting2("PraidingP") : 0;
      I = getPageSetting2("PraidingI") > 0 ? getPageSetting2("PraidingI") : 0;
    }
    if (game.global.challengeActive == "Daily") {
      HD = getPageSetting2("dPraidingHD");
      P = getPageSetting2("dPraidingP") > 0 ? getPageSetting2("dPraidingP") : 0;
      I = getPageSetting2("dPraidingI") > 0 ? getPageSetting2("dPraidingI") : 0;
    }
    var go = false;
    if (HD <= 0) {
      go = true;
  ...
  globalThis.pMap1 = void 0;
  globalThis.pMap2 = void 0;
  globalThis.pMap3 = void 0;
  globalThis.pMap4 = void 0;
  globalThis.pMap5 = void 0;
  globalThis.repMap1 = void 0;
  globalThis.repMap2 = void 0;
  globalThis.repMap3 = void 0;
  globalThis.repMap4 = void 0;
  globalThis.repMap5 = void 0;
  ...
  function Praiding() {
    ...
    if (getPageSetting2("Praidingzone").length) {
      if (getPageSetting2("Praidingzone").includes(game.global.world) && (cell <= 1 || cell > 1 && game.global.lastClearedCell + 1 >= cell) && !prestraid && !failpraid) {
        prestraidon = true;
        if (getPageSetting2("AutoMaps") == 1 && !prestraid && !failpraid) {
        ...
        if (!game.global.preMapsActive && !game.global.mapsActive && !prestraid) {
          ...
          debug2("Beginning Prestige Raiding...");
        }
        ...
        if (game.global.preMapsActive && !game.global.mapsActive && !prestraid) {
          debug2("Map Loop");
          if (pcheckmap5() == true && pcheck5() == true && pMap5 == void 0 && !mapbought5 && game.global.preMapsActive && !prestraid) {
            debug2("Check complete for 5th map");
            plusPres5();
            if (updateMapCost(true) <= game.resources.fragments.owned) {
              buyMap();
          ...
          if (pcheckmap4() == true && pcheck4() == true && pMap4 == void 0 && !mapbought4 && game.global.preMapsActive && !prestraid) {
            debug2("Check complete for 4th map");
            plusPres4();
            if (updateMapCost(true) <= game.resources.fragments.owned) {
              buyMap();
          ...
          if (pcheckmap3() == true && pcheck3() == true && pMap3 == void 0 && !mapbought3 && game.global.preMapsActive && !prestraid) {
            debug2("Check complete for 3rd map");
            plusPres3();
            if (updateMapCost(true) <= game.resources.fragments.owned) {
              buyMap();
          ...
          if (pcheckmap2() == true && pcheck2() == true && pMap2 == void 0 && !mapbought2 && game.global.preMapsActive && !prestraid) {
            debug2("Check complete for 2nd map");
            plusPres2();
            if (updateMapCost(true) <= game.resources.fragments.owned) {
              buyMap();
          ...
          if (pcheckmap1() == true && pcheck1() == true && pMap1 == void 0 && !mapbought1 && game.global.preMapsActive && !prestraid) {
            debug2("Check complete for 1st map");
            plusPres1();
            if (updateMapCost(true) <= game.resources.fragments.owned) {
              buyMap();
          ...
          if (!mapbought1 && !mapbought2 && !mapbought3 && !mapbought4 && !mapbought5) {
            if (getPageSetting2("AutoMaps") == 0 && !prestraid) {
              ...
              pMap4 = void 0;
              pMap5 = void 0;
              debug2("Failed to Prestige Raid. Looks like you can't afford to or you are too weak or you have limited yourself in a P/I zone. ");
            }
            return;
          }
        }
        if (game.global.preMapsActive && !game.global.mapsActive && mapbought1 && pMap1 != void 0 && !prestraid) {
          debug2("running map 1");
          selectMap(pMap1);
          runMap();
          repMap1 = pMap1;
        ...
        if (game.global.preMapsActive && !game.global.mapsActive && mapbought2 && pMap2 != void 0 && !prestraid) {
          debug2("running map 2");
          selectMap(pMap2);
          runMap();
          repMap2 = pMap2;
        ...
        if (game.global.preMapsActive && !game.global.mapsActive && mapbought3 && pMap3 != void 0 && !prestraid) {
          debug2("running map 3");
          selectMap(pMap3);
          runMap();
          repMap3 = pMap3;
        ...
        if (game.global.preMapsActive && !game.global.mapsActive && mapbought4 && pMap4 != void 0 && !prestraid) {
          debug2("running map 4");
          selectMap(pMap4);
          runMap();
          repMap4 = pMap4;
        ...
        if (game.global.preMapsActive && !game.global.mapsActive && mapbought5 && pMap5 != void 0 && !prestraid) {
          debug2("running map 5");
          selectMap(pMap5);
          runMap();
          repMap5 = pMap5;
    ...
    if (game.global.preMapsActive && (mapbought1 || mapbought2 || mapbought3 || mapbought4 || mapbought5) && pMap1 == void 0 && pMap2 == void 0 && pMap3 == void 0 && pMap4 == void 0 && pMap5 == void 0 && !prestraid && !failpraid) {
      prestraid = true;
      failpraid = false;
      mapbought1 = false;
      mapbought2 = false;
    ...
    if (getPageSetting2("AutoMaps") == 0 && game.global.preMapsActive && prestraid && !failpraid && prestraidon) {
      praidDone = true;
      prestraidon = false;
      if (repMap1 != void 0) {
      ...
      pMap4 = void 0;
      pMap5 = void 0;
      debug2("Prestige raiding successful!");
      debug2("Turning AutoMaps back on");
    }
    if (getPageSetting2("Praidingzone").every(isBelowThreshold)) {
      prestraid = false;
      failpraid = false;
      prestraidon = false;
      mapbought1 = false;
  ...
  function PraidHarder() {
    ...
    if (game.global.challengeActive == "Daily") {
      ...
      farmFragments = getPageSetting2("dPraidFarmFragsZ").includes(game.global.world);
      praidBeforeFarm = getPageSetting2("dPraidBeforeFarmZ").includes(game.global.world);
      ...
      farmFragments = getPageSetting2("PraidFarmFragsZ").includes(game.global.world);
      praidBeforeFarm = getPageSetting2("PraidBeforeFarmZ").includes(game.global.world);
      cell = getPageSetting2("Praidingcell") > 0 ? getPageSetting2("Praidingcell") : 0;
    }
    pRaidIndex = getPageSetting2(praidSetting).indexOf(game.global.world);
    if (pRaidIndex == -1 || typeof getPageSetting2(maxPraidZSetting)[pRaidIndex] === "undefined") maxPlusZones = plusMapToRun(game.global.world);
    else maxPlusZones = getPageSetting2(maxPraidZSetting)[pRaidIndex] - game.global.world;
    ...
    if (getPageSetting2(praidSetting).length) {
      if (getPageSetting2(praidSetting).includes(game.global.world) && game.global.lastClearedCell + 1 >= cell && !prestraid && !failpraid && !shouldFarmFrags) {
        debug2("Beginning Praiding");
        ...
        if (curPlusZones >= 0 && (praidBeforeFarm || shouldFarmFrags == false)) {
          ...
          prestraidon = false;
          praidDone = true;
          debug2("Failed to prestige raid. Looks like you can't afford to.");
        } else {
          debug2("Turning AutoMaps back on");
          autoTrimpSettings["AutoMaps"].value = 1;
          game.options.menu.repeatUntil.enabled = 0;
    ...
    if (game.global.preMapsActive && prestraid && !failpraid && !shouldFarmFrags && prestraidon) {
      prestraidon = false;
      praidDone = true;
      debug2("Prestige raiding successful! - recycling Praid map");
      if (pMap) recycleMap(getMapIndex(pMap));
    ...
    if (!getPageSetting2(praidSetting).includes(game.global.world)) {
      prestraid = false;
      failpraid = false;
      prestraidon = false;
      shouldFarmFrags = false;
  ...
  function BWraiding() {
    ...
    if (getPageSetting2("AutoMaps") == 0 && game.global.preMapsActive && bwraided && !failbwraid) {
      autoTrimpSettings["AutoMaps"].value = 1;
      debug2("Turning AutoMaps back on");
    }
    if (!isBWRaidZ) {
  ...
  globalThis.dpMap1 = void 0;
  globalThis.dpMap2 = void 0;
  globalThis.dpMap3 = void 0;
  globalThis.dpMap4 = void 0;
  globalThis.dpMap5 = void 0;
  globalThis.drepMap1 = void 0;
  globalThis.drepMap2 = void 0;
  globalThis.drepMap3 = void 0;
  globalThis.drepMap4 = void 0;
  globalThis.drepMap5 = void 0;
  ...
  function dailyPraiding() {
    ...
    if (getPageSetting2("dPraidingzone").length) {
      if (getPageSetting2("dPraidingzone").includes(game.global.world) && (cell <= 1 || cell > 1 && game.global.lastClearedCell + 1 >= cell) && !dprestraid && !dfailpraid) {
        dprestraidon = true;
        if (getPageSetting2("AutoMaps") == 1 && !dprestraid && !dfailpraid) {
          autoTrimpSettings["AutoMaps"].value = 0;
        }
        ...
        if (game.global.preMapsActive && !game.global.mapsActive && !dprestraid) {
          debug2("Map Loop");
          if (pcheckmap5() == true && pcheck5() == true && dpMap5 == void 0 && !dmapbought5 && game.global.preMapsActive && !dprestraid) {
            debug2("Check complete for 5th map");
            plusPres5();
            if (updateMapCost(true) <= game.resources.fragments.owned) {
              buyMap();
          ...
          if (pcheckmap4() == true && pcheck4() == true && dpMap4 == void 0 && !dmapbought4 && game.global.preMapsActive && !dprestraid) {
            debug2("Check complete for 4th map");
            plusPres4();
            if (updateMapCost(true) <= game.resources.fragments.owned) {
              buyMap();
          ...
          if (pcheckmap3() == true && pcheck3() == true && dpMap3 == void 0 && !dmapbought3 && game.global.preMapsActive && !dprestraid) {
            debug2("Check complete for 3rd map");
            plusPres3();
            if (updateMapCost(true) <= game.resources.fragments.owned) {
              buyMap();
          ...
          if (pcheckmap2() == true && pcheck2() == true && dpMap2 == void 0 && !dmapbought2 && game.global.preMapsActive && !dprestraid) {
            debug2("Check complete for 2nd map");
            plusPres2();
            if (updateMapCost(true) <= game.resources.fragments.owned) {
              buyMap();
          ...
          if (pcheckmap1() == true && pcheck1() == true && dpMap1 == void 0 && !dmapbought1 && game.global.preMapsActive && !dprestraid) {
            debug2("Check complete for 1st map");
            plusPres1();
            if (updateMapCost(true) <= game.resources.fragments.owned) {
              buyMap();
          ...
          if (!dmapbought1 && !dmapbought2 && !dmapbought3 && !dmapbought4 && !dmapbought5) {
            if (getPageSetting2("AutoMaps") == 0 && !dprestraid) {
              autoTrimpSettings["AutoMaps"].value = 1;
              game.options.menu.repeatUntil.enabled = 0;
              dprestraidon = false;
              dfailpraid = true;
        ...
        if (game.global.preMapsActive && !game.global.mapsActive && dmapbought1 && dpMap1 != void 0 && !dprestraid) {
          debug2("running map 1");
          selectMap(dpMap1);
          runMap();
          drepMap1 = dpMap1;
        ...
        if (game.global.preMapsActive && !game.global.mapsActive && dmapbought2 && dpMap2 != void 0 && !dprestraid) {
          debug2("running map 2");
          selectMap(dpMap2);
          runMap();
          drepMap2 = dpMap2;
        ...
        if (game.global.preMapsActive && !game.global.mapsActive && dmapbought3 && dpMap3 != void 0 && !dprestraid) {
          debug2("running map 3");
          selectMap(dpMap3);
          runMap();
          drepMap3 = dpMap3;
        ...
        if (game.global.preMapsActive && !game.global.mapsActive && dmapbought4 && dpMap4 != void 0 && !dprestraid) {
          debug2("running map 4");
          selectMap(dpMap4);
          runMap();
          drepMap4 = dpMap4;
        ...
        if (game.global.preMapsActive && !game.global.mapsActive && dmapbought5 && dpMap5 != void 0 && !dprestraid) {
          debug2("running map 5");
          selectMap(dpMap5);
          runMap();
          drepMap5 = dpMap5;
    ...
    if (getPageSetting2("AutoMaps") == 0 && game.global.preMapsActive && dprestraid && !dfailpraid && dprestraidon) {
      dpraidDone = true;
      dprestraidon = false;
      if (drepMap1 != void 0) {
    ...
    if (getPageSetting2("dPraidingzone").every(isBelowThreshold)) {
      dprestraid = false;
      dfailpraid = false;
      dprestraidon = false;
      dmapbought1 = false;
  ...
  __export(settings_menu_exports, {
    autoPlusSettingsMenu: () => autoPlusSettingsMenu,
    autoToggle: () => autoToggle,
    automationMenuInit: () => automationMenuInit2,
    automationMenuSettingsInit: () => automationMenuSettingsInit2,
    createTabContents: () => createTabContents,
    createTabs: () => createTabs,
    initializeAllTabs: () => initializeAllTabs2,
    maximizeAllTabs: () => maximizeAllTabs,
    minimizeAllTabs: () => minimizeAllTabs,
    modifyParentNode: () => modifyParentNode2,
    toggleAutoMaps: () => toggleAutoMaps,
    toggleTab: () => toggleTab
  });
  var addTabsDiv;
  var addtabsUL;
  function automationMenuInit2() {
  ...
  function modifyParentNode2(setting, id) {
    var elem = document.getElementById(id).parentNode.parentNode.children;
    for (var i = 0; i < elem.length; i++) {
      if (document.getElementById(id).parentNode.parentNode.children[i].children[0] === void 0) {
        continue;
      } else {
  ...
  // src/modules/settings-visibility.ts
  var settings_visibility_exports = {};
  __export(settings_visibility_exports, {
  ...
  function updateCustomButtons2() {
    ...
    function toggleElem(elem2, showHide) {
    ...
    function turnOff(elem2) {
      toggleElem(elem2, false);
    }
  ...
  var settings_defs_exports = {};
  __export(settings_defs_exports, {
    initializeAllSettings: () => initializeAllSettings2
  });
  function initializeAllSettings2() {
    ...
    createSetting("RdHeliumHourChallenge", "DP: Challenge", "Automatically portal into this challenge when using radon per hour or custom autoportal in dailies when there are none left. Custom portals after cell 100 of the zone specified. Do not choose a challenge if you havent unlocked it. ", "dropdown", "None", ["None", "Bubl\xE9", "Melt", "Quagmire", "Archaeology", "Insanity", "Nurture", "Alchemy", "Hypothermia"], "Daily");
    ...
    createSetting("BuyArmorNew", ["Armor: Buy Neither", "Armor: Buy Both", "Armor: Prestiges", "Armor: Levels"], "AutoBuys Prestiges and Levels up the most cost efficient Armor available. Gymystic buying is controlled under this setting's prestige option", "multitoggle", 1, null, "Gear");
    createSetting("BuyWeaponsNew", ["Weapons: Buy Neither", "Weapons: Buy Both", "Weapons: Prestiges", "Weapons: Levels"], "AutoBuys Prestiges and Levels up the most cost efficient Weapon available.", "multitoggle", 1, null, "Gear");
  ...
  var preset_Zek550 = [700, 2.8, 70, 1.4, 1, 2.2, 7.5, 2.8, 8, 3e-3, 1, 50, 80, 45];
  ...
  var presetListHtml = "<option id='preset_Zek059'>Zeker0 (z1-59)</option><option id='preset_Zek100'>Zeker0 (z60-100)</option><option id='preset_Zek180'>Zeker0 (z101-180)</option><option id='preset_Zek229'>Zeker0 (z181-229)</option><option id='preset_Zek299'>Zeker0 (z230-299)</option><option id='preset_Zek399'>Zeker0 (z300-399)</option><option id='preset_Zek449'>Zeker0 (z400-449)</option><option id='preset_Zek450'>Zeker0 (z450-500)</option><option id='preset_Zek500'>Zeker0 (z501-549)</option><option id='preset_Zek500'>Zeker0 (z550+)</option><option id='preset_space'>--------------</option><option id='customPreset'>CUSTOM ratio</option></select>";
  ...
  function ensurePerksGUI(selectId, build) {
  ...
  AutoPerks.setDefaultRatios = function() {
    ...
    var currentPerk;
    for (var i = 0; i < $perkRatioBoxes.length; i++) {
      currentPerk = AutoPerks.getPerkByName($perkRatioBoxes[i].id.substring(0, $perkRatioBoxes[i].id.length - 5));
  ...
  AutoPerks.clickAllocate = function() {
    ...
    if (preSpentHe)
      debug2("AutoPerks: Your existing fixed-perks reserve Helium: " + prettify(preSpentHe), "perks");
    ...
    if (Number.isNaN(remainingHelium))
      debug2("AutoPerks: Major Error: Reading your Helium amount. " + remainingHelium, "perks");
    var result;
    if (getPageSetting2("fastallocate") == true)
      result = AutoPerks.spendHelium2(remainingHelium);
    else
      result = AutoPerks.spendHelium(remainingHelium);
    if (result == false) {
      debug2("AutoPerks: Major Error: Make sure all ratios are set properly.", "perks");
      return;
    }
    var perks = AutoPerks.getOwnedPerks();
    AutoPerks.applyCalculations(perks, remainingHelium);
    debug2("AutoPerks: Auto-Allocate Finished.", "perks");
  };
  ...
  AutoPerks.spendHelium = function(helium) {
    debug2("Beginning AutoPerks1 calculate how to spend " + prettify(helium) + " Helium... This could take a while...", "perks");
    if (helium < 0) {
      debug2("AutoPerks: Major Error - Not enough helium to buy fixed perks.", "perks");
      return false;
    }
    if (Number.isNaN(helium)) {
      debug2("AutoPerks: Major Error - Helium is Not a Number!", "perks");
      return false;
    }
    ...
    if (effQueue.size < 1) {
      debug2("All Perk Ratios were 0, or some other error.", "perks");
      return false;
    }
    var i = 0;
    ...
    if ($selector != null && $selector.value != "None") {
      ...
      debug2("AutoPerks1: Dump Perk " + AutoPerks.capitaliseFirstLetter(dumpPerk.name) + " level post-dump: " + dumpPerk.level + " Helium Dumped: " + prettify(dumpresults) + " He.", "perks");
    }
    ...
    debug2("AutoPerks1: Pass two complete. Round 2 cleanup spend of : " + prettify(r2results), "perks");
  };
  AutoPerks.spendHelium2 = function(helium) {
    debug2("Beginning AutoPerks2 calculate how to spend " + prettify(helium) + " Helium... This could take a while...", "perks");
    if (helium < 0) {
      debug2("AutoPerks: Major Error - Not enough helium to buy fixed perks.", "perks");
      return false;
    }
    ...
    if ($selector != null && $selector.value != "None") {
      ...
      var dumpresults = heb4dump - helium;
      debug2("AutoPerks2: Dump Perk " + AutoPerks.capitaliseFirstLetter(dumpPerk.name) + " level post-dump: " + dumpPerk.level + " Helium Dumped: " + prettify(dumpresults) + " He.", "perks");
    }
    var heB4round2 = helium;
    while (effQueue.size > 1) {
      mostEff = effQueue.poll();
    ...
    var r2results = heB4round2 - helium;
    debug2("AutoPerks2: Pass Two Complete. Cleanup Spent Any Leftover Helium: " + prettify(r2results) + " He.", "perks");
  };
      ...
      for (var i in perks) {
        ...
        if (getPortalUpgradePrice(capitalized) <= remainingHelium || perks[i].fixed) {
          if (MODULES["perks"].showDetails)
            debug2("AutoPerks-Respec Buying: " + capitalized + " " + perks[i].level, "perks");
          buyPortalUpgrade(capitalized);
        } else if (MODULES["perks"].showDetails)
          debug2("AutoPerks-Respec Error Couldn't Afford Asked Perk: " + capitalized + " " + perks[i].level, "perks");
      }
      game.global.buyAmt = preBuyAmt3;
      numTab(1, true);
      cancelTooltip();
    } else {
      debug2("A Respec would be required and is not available. You used it already, try again next portal.", "perks");
  ...
  AutoPerks.applyCalculations = function(perks, remainingHelium) {
    ...
    for (var i in perks) {
      ...
      if (game.global.buyAmt < 0) {
        needsRespec = true;
        if (MODULES["perks"].showDetails)
          debug2("AutoPerks RESPEC Required for: " + capitalized + " " + game.global.buyAmt, "perks");
      } else if (game.global.buyAmt > 0) {
        if (MODULES["perks"].showDetails)
          debug2("AutoPerks-NoRespec Adding: " + capitalized + " " + game.global.buyAmt, "perks");
        buyPortalUpgrade(capitalized);
      }
    }
    game.global.buyAmt = preBuyAmt3;
    ...
    cancelTooltip();
    if (needsRespec) {
      debug2("AutoPerks - A Respec is required. Trying respec...", "perks");
  ...
  AutoPerks.initializePerks = function() {
    var siphonology = new AutoPerks.FixedPerk("siphonology", 1e5, 3, 3);
    var anticipation = new AutoPerks.FixedPerk("anticipation", 1e3, 10, 10);
    ...
    var coordinated = new AutoPerks.VariablePerk("coordinated", 15e4, true, 8, 0.1);
    var resourceful = new AutoPerks.VariablePerk("resourceful", 5e4, true, 9, 0.05);
    var overkill = new AutoPerks.VariablePerk("overkill", 1e6, true, 10, 5e-3, 30);
    var capable = new AutoPerks.FixedPerk("capable", 1e8, 0, 10, "fluffy");
    var cunning = new AutoPerks.VariablePerk("cunning", 1e11, false, 11, 0.05);
    var curious = new AutoPerks.VariablePerk("curious", 1e14, false, 12, 0.05);
    var classy = new AutoPerks.VariablePerk("classy", 1e17, false, 13, 0.05, 75);
    var toughness_II = new AutoPerks.ArithmeticPerk("toughness_II", 2e4, 500, 0.01, toughness);
    var power_II = new AutoPerks.ArithmeticPerk("power_II", 2e4, 500, 0.01, power);
    var motivation_II = new AutoPerks.ArithmeticPerk("motivation_II", 5e4, 1e3, 0.01, motivation);
    var carpentry_II = new AutoPerks.ArithmeticPerk("carpentry_II", 1e5, 1e4, 25e-4, carpentry);
    var looting_II = new AutoPerks.ArithmeticPerk("looting_II", 1e5, 1e4, 25e-4, looting);
  ...
  AutoPerks.getSomePerks = function(fixed, variable, tier2, allperks) {
    ...
    for (var i in AutoPerks.perkHolder) {
      ...
      if (fixed && AutoPerks.perkHolder[i].fixed || variable && !AutoPerks.perkHolder[i].fixed || tier2 && AutoPerks.perkHolder[i].type == "linear" && !AutoPerks.perkHolder[i].fluffy || allperks) {
  ...
  var RpresetListHtml = "<option id='preset_RZek059'>Zek (z1-59)</option><option id='preset_RZekmelt'>Zek (Melt)</option><option id='preset_RZekquag'>Zek (Quag)</option><option id='preset_Rspace'>--------------</option><option id='customPreset'>CUSTOM ratio</option></select>";
  ...
  RAutoPerks.clickAllocate = function() {
    ...
    if (preSpentRn)
      debug2("RAutoPerks: Your existing fixed-perks reserve Radon: " + prettify(preSpentRn), "perks");
    ...
    if (Number.isNaN(remainingRadon))
      debug2("RAutoPerks: Major Error: Reading your Radon amount. " + remainingRadon, "perks");
    var result;
    if (getPageSetting2("fastallocate") == true)
    ...
    if (result == false) {
      debug2("RAutoPerks: Major Error: Make sure all ratios are set properly.", "perks");
      return;
    }
    var perks = RAutoPerks.getOwnedPerks();
    RAutoPerks.applyCalculations(perks, remainingRadon);
    debug2("RAutoPerks: Auto-Allocate Finished.", "perks");
  };
  ...
  RAutoPerks.spendRadon = function(radon) {
    debug2("Beginning RAutoPerks1 calculate how to spend " + prettify(radon) + " Radon... This could take a while...", "perks");
    if (radon < 0) {
      debug2("RAutoPerks: Major Error - Not enough radon to buy fixed perks.", "perks");
      return false;
    }
    if (Number.isNaN(radon)) {
      debug2("RAutoPerks: Major Error - Radon is Not a Number!", "perks");
      return false;
    }
    ...
    if ($selector != null && $selector.value != "None") {
      ...
      debug2("RAutoPerks1: Dump Perk " + RAutoPerks.capitaliseFirstLetter(RdumpPerk.name) + " level post-dump: " + RdumpPerk.radLevel + " Radon Dumped: " + prettify(dumpresults) + " Rn.", "perks");
    }
    ...
    debug2("RAutoPerks1: Pass two complete. Round 2 cleanup spend of : " + prettify(r2results), "perks");
  };
  RAutoPerks.spendRadon2 = function(radon) {
    debug2("Beginning RAutoPerks2 calculate how to spend " + prettify(radon) + " Radon... This could take a while...", "perks");
    if (radon < 0) {
      debug2("RAutoPerks: Major Error - Not enough radon to buy fixed perks.", "perks");
      return false;
    }
    ...
    if ($selector != null && $selector.value != "None") {
      ...
      var dumpresults = heb4dump - radon;
      debug2("RAutoPerks2: Dump Perk " + RAutoPerks.capitaliseFirstLetter(RdumpPerk.name) + " level post-dump: " + RdumpPerk.radLevel + " Radon Dumped: " + prettify(dumpresults) + " Rn.", "perks");
    }
    var heB4round2 = radon;
    while (effQueue.size > 1) {
      mostEff = effQueue.poll();
    ...
    var r2results = heB4round2 - radon;
    debug2("RAutoPerks2: Pass Two Complete. Cleanup Spent Any Leftover Radon: " + prettify(r2results) + " He.", "perks");
  };
      ...
      for (var i in perks) {
        ...
        if (getPortalUpgradePrice(capitalized) <= remainingRadon || perks[i].fixed) {
          if (MODULES["perks"].RshowDetails)
            debug2("RAutoPerks-Respec Buying: " + capitalized + " " + perks[i].radLevel, "perks");
          buyPortalUpgrade(capitalized);
        } else if (MODULES["perks"].RshowDetails)
          debug2("RAutoPerks-Respec Error Couldn't Afford Asked Perk: " + capitalized + " " + perks[i].radLevel, "perks");
      }
      game.global.buyAmt = preBuyAmt3;
      numTab(1, true);
      cancelTooltip();
  ...
  RAutoPerks.applyCalculations = function(perks, remainingRadon) {
    ...
    for (var i in perks) {
      ...
      if (game.global.buyAmt < 0) {
        needsRespec = true;
        if (MODULES["perks"].RshowDetails)
          debug2("RAutoPerks RESPEC Required for: " + capitalized + " " + game.global.buyAmt, "perks");
      } else if (game.global.buyAmt > 0) {
        if (MODULES["perks"].RshowDetails)
          debug2("RAutoPerks-NoRespec Adding: " + capitalized + " " + game.global.buyAmt, "perks");
        buyPortalUpgrade(capitalized);
      }
    }
    game.global.buyAmt = preBuyAmt3;
    ...
    cancelTooltip();
    if (needsRespec) {
      debug2("RAutoPerks - A Respec is required. Trying respec...", "perks");
      var whichscreen = game.global.viewingUpgrades;
      cancelPortal();
      if (whichscreen)
        viewPortalUpgrades();
  ...
  RAutoPerks.initializePerks = function() {
    ...
    var hunger = new RAutoPerks.FixedPerk("hunger", 1e6, 30);
    var observation = new RAutoPerks.FixedPerk("observation", 5e18, 50);
    ...
    var tenacity = new RAutoPerks.VariablePerk("tenacity", 5e7, true, 11, 0.1, 40);
    var greed = new RAutoPerks.VariablePerk("greed", 1e10, true, 12, 0.1, 40);
    var frenzy = new RAutoPerks.VariablePerk("frenzy", 1e15, true, 13, 0.1);
    var championism = new RAutoPerks.VariablePerk("championism", 1e9, true, 14, 0.1);
    equality.exprate = 1.5;
    observation.exprate = 2;
    championism.exprate = 5;
    RAutoPerks.perkHolder = [
      range,
      agility,
      bait,
      trumps,
  ...
  RAutoPerks.getSomePerks = function(fixed, variable, tier2, allperks) {
    ...
    for (var i in RAutoPerks.perkHolder) {
      ...
      if (fixed && RAutoPerks.perkHolder[i].fixed || variable && !RAutoPerks.perkHolder[i].fixed || tier2 && RAutoPerks.perkHolder[i].type == "linear" && !RAutoPerks.perkHolder[i].fluffy || allperks) {
  ...
  (function(M) {
    ...
    function Update() {
      ...
      for (var i = 0; i < $cells.length; i++) {
        ...
        } else if (M["fightinfo"].exotics.indexOf(cell.name) > -1) {
          if (cell.special.length === 0)
        ...
        if (cell.name.toLowerCase().indexOf("poison") > -1) {
          if (cell.special.length === 0)
            $cell.innerHTML = '<span class="glyphicon glyphicon-flask"></span> ';
          $cell.title = cell.name;
          $cell.style.textShadow = "0px 0px 10px #ffffff";
        }
        if (cell.name.toLowerCase().indexOf("wind") > -1) {
          if (cell.special.length === 0)
            $cell.innerHTML = '<span class="icomoon icon-air"></span> ';
          $cell.title = cell.name;
          $cell.style.textShadow = "0px 0px 10px #ffffff";
        }
        if (cell.name.toLowerCase().indexOf("ice") > -1) {
          if (cell.special.length === 0)
  ...
  (function(M, W) {
    ...
    M["performance"].UpdateAFKOverlay = function UpdateAFKOverlay() {
      M["performance"].AFKOverlayZone.innerText = "Zone: " + game.global.world + (game.global.mapsActive ? " (Map: " + (getCurrentMapObject().level - game.global.world >= 0 ? "+" : "") + (getCurrentMapObject().level - game.global.world) + " " + (getCurrentMapObject().bonus !== void 0 ? getCurrentMapObject().bonus : "") + ")" : "");
      if (game.global.universe == 1) {
